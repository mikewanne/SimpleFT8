...
{
    int is_ft4_like = (wf->protocol == FTX_PROTOCOL_FT4 || wf->protocol == FTX_PROTOCOL_FT2);
    int (*sync_fun)(const ftx_waterfall_t*, const ftx_candidate_t*) = is_ft4_like ? ft4_sync_score : ft8_sync_score;
    int num_tones = is_ft4_like ? 4 : 8;
...
bool ftx_decode_candidate(const ftx_waterfall_t* wf, const ftx_candidate_t* cand, int max_iterations, ftx_message_t* message, ftx_decode_status_t* status)
{
    ...
    if (wf->protocol == FTX_PROTOCOL_FT4 || wf->protocol == FTX_PROTOCOL_FT2)
    {
        // ‘[..] for FT4/FT2, in order to avoid transmitting a long string of zeros when sending CQ messages,
