...
class CycleMixin:
    ...
    def _p128_recently_completed_block(self, caller: str) -> bool:
        ...
        P138 (26.05.2026, Mike-Field-Bug Spec-Umkehr): die P129-Whitelist
        (73/RR73 trotz Cooldown durchlassen) ist ENTFERNT. Mike sah
        nach ✓ einen verspäteten 73-Eintrag der ins neue QSO „herein-
        rutschte" — „beendet ist beendet". Der Cooldown-Stempel wird in
        `_on_qso_complete` (mw_qso.py:557) gesetzt, also exakt zum
        ✓-Zeitpunkt. Damit gilt automatisch:
        - vor ✓: kein Cooldown-Eintrag → 73 kommt durch (gewünscht)
        - nach ✓: Cooldown aktiv → alles geblockt inkl. 73 (gewünscht)
