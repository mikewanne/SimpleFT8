...
**Aktueller Stand:** **v0.97.86 Session 21.05.2026 — P109: X-Button im QSO-Detail-Overlay schliesst auch Logbuch-Tab.** Mike-Beobachtung 21.05.: Klick auf X schloss nur das Overlay, Logbuch-Tab blieb offen. Spec: X = QSO-Button-Verhalten (Tab + Overlay beide zurück auf QSO-Live). Fix 1-LOC in `ui/main_window.py:679-682`: btn_close-Click ruft jetzt `qso_panel.tabs.setCurrentIndex(0)` statt direkt `_right_stack.setCurrentIndex(0)` — der existierende `_on_qso_tab_changed(0)`-Handler erledigt Stack-Switch automatisch (Single-Source-of-Truth). Delete-Pfad bewusst unverändert (R1: „User sieht Refresh"). Workflow V1 → V2 → R1 („1-LOC sauber, Delete nicht ändern, 2 Tests reichen") → V3 → Code → 2 Tests T1-T2 → Final-R1 V4-pro „Push-bereit (GRÜN)" 0 Blocker. **V4-pro 45-Cycle-Bilanz: 0 Halluzinationen.** Tests 1714→1716 (+2). P66-Tests (`_on_qso_tab_changed`-Handler) bleiben grün — Handler unverändert. **Vorgänger v0.97.85 P108** Kommentar-Feld aus QSO-Detail-Overlay entfernt. Mike-Beobachtung nach P106 (WSJT-X-Minimal-ADIF) + QRZ-Bestätigungen wieder OK: das Feld „Kommentar:" im QSO-Detail-Overlay zeigte noch alte „SimpleFT8 v1.0"-Inhalte, war aber seit P106 funktionslos (COMMENT wird nicht mehr exportiert; auch vorher schon war das Widget read-only — Save-Pfad las den Edit-Wert nie aus). Fix: 10 LOC raus aus `ui/qso_detail_overlay.py` (Label + QLineEdit + Grid-Add Z. 117-125 + setText Z. 190). Voller Workflow V1 → V2-Self-Review → R1-DeepSeek („grün durch und durch, nur GELB Tests") → V3 → Code → 4 Tests T1-T4 → Final-R1 V4-pro „Push-bereit, kein Rot/Orange-Blocker, Go". **V4-pro 44-Cycle-Bilanz: 0 Halluzinationen.** Tests 1710→1714 (+4): T1 comment_edit-Attribut entfernt, T2 load_qso() Smoke mit + ohne COMMENT, T3 _on_save emittiert _qso_data unverändert, T4 Label „Kommentar:" nicht mehr im Source. Alte ADIF-Records bleiben unangetastet — Lese-Pfad zeigt sie nur nicht mehr an, Export-Pfad (P107) filtert COMMENT ohnehin via `_rewrite_minimal`. **Vorgänger v0.97.73 P101** TUNE-Override Variante B + QMenu Padding-Korrektur. Mike-Field-Test 21.05. nach P100 zeigte 2 Bugs: (A) Rechtsklick auf TUNE-Button startet das Tunen nicht direkt — Guard `if btn_tune.isChecked()` war stale True nach Auto-Stop-Token-Race vom Vorgänger-TUNE → Override interpretierte als „läuft → stop". (B) QMenu padding aus P100 asymmetrisch: links zu viel, Häkchen am Rand. Voller Workflow V1→V2(8 Findings)→R1-DeepSeek→V3+Code+Tests→Final-R1 (1 ROT). Fix A: Guard auf `_tune_active` umgestellt, Variante B „Dauer-Switch in einem Rutsch" (bei aktivem TUNE → synchron stop + sofort start mit neuer Dauer). 4 Diagnose-Prints `[P101]` zur Signal-Verifikation für Mike-Field-Test. Final-R1-Catch: `_tune_start` cancelt jetzt latenten Post-Check-Token vom Vorgänger + leert `_fwdpwr_samples` (sonst Race-Auswertung mitten im neuen TUNE). Fix B: Padding symmetrisch 20/20 + Indicator explizit `subcontrol-position: left center` + `margin-left: 0` (R1: macOS-Theme kann Default-Position rechts rendern). Gilt für qso_panel.py `_build_columns_menu` + rx_panel.py 2× (Spalten + Länder-Filter). TUNE-Override-Menü unverändert (keine checkable Items). Tests 1683→1697 (+14): P95 t4/t5 angepasst (Switch statt Stop), P100 t8/t9 angepasst (neue Padding-Werte), P101 t1-t5 neu. Field-Test mit Radio pending — Mike sieht Diagnose-Prints im Terminal, wenn Override aufgerufen wird. **Vorgänger v0.97.72 P100** QSO-Button Kontextmenü + Padding-Fix. **Vorgänger v0.97.70 P98** Retry-Limits 3→5 + Auto-Hunt-Bugfix. Mike-Field-Test 20.05. (Screenshots TA4SSK + DG8DBW): 3 Retries waren zu knapp. TA4SSK Antwort beim 3. (!) Versuch gerade noch; DG8DBW 2× RR73 dann Timeout. DeepSeek-R1-Brainstorm-Konsens: beide Limits auf 5. Code: `MAX_RR73_RETRIES = 3→5`, `QSOData.max_calls` Default 3→5, alle Settings-Fallbacks `get("max_calls", 3)→5`, `config/settings.py` initial-Default `99→5`, Settings-Dialog Hint+Reset angepasst. **R1-F2 Bugfix**: `ui/mw_cycle.py:515` Hartcodierung `max_calls = 3` → jetzt `settings.get("max_calls", 5)` (Auto-Hunt ignorierte vorher das User-Setting!). **R1-F1 → P99-Folge-Ticket:** DG8DBW-Pfad geht über `on_message_received`-WAIT_RR73-Branch (Gegenstation wiederholt R-Report) — der hat KEINEN Retry-Counter, kann unbegrenzt senden. P98 deckt nur „leerer Slot"-Pfad ab. **R1-F3:** Tests importieren `MAX_RR73_RETRIES` statt Hartcodierung. Workflow voll: V1 → V2 (4 Klärungen) → R1 (3 Findings) → V3+Code+10 Tests T1-T10 → Final-R1 V4-pro „kann gepusht werden" 0 Blocker. **V4-pro 43-Cycle-Bilanz: 0 Halluzinationen, 1 echter Bug gefangen.** Tests 1671→1681 (+10 P98). Field-Test mit Radio pending F1-F3. **Vorgänger v0.97.69 P97** Collapsed-Card Status-Suffix. Mike-Wunsch 20.05.: bei eingeklappter Kachel im Header neben „ANTENNE"/„RADIO" aktuellen Modus bzw. Sendeleistung sichtbar lassen — Mike weiß auch mit collapsed Kachel welcher Modus aktiv ist. Antenne: „— Normal" / „— Diversity Standard" / „— Diversity DX". Radio: „— 70 W". Schriftfarbe wie Header (#55BBAA für Antenne, #00aacc für Radio) — dezent. Status-Labels in beiden Cards (`_AntenneCard.lbl_ant_status` + `_RadioCard.lbl_radio_status`), 2 SOT-Tracker (`_current_scoring_mode`, `_current_power_watts`), 2 Refresh-Helper mit Hooks an `set_rx_mode`/`update_diversity_ratio`/`set_power_preset`/`_on_power_preset_clicked`. 10 Tests T1-T10. Tests 1661→1671 (+10 P97). Field-Test ohne Radio. **Vorgänger v0.97.68 P96** Power-Button-Farbe permanent. Mike-Beobachtung 20.05. nach P95: nach Auswahl einer Leistung (z.B. 70W) verschwand die Hintergrundfarbe beim Mouse-Over — „wie Toggle-Aus". Ursache Qt-CSS-Spezifität: `:hover` stand NACH `:checked` → last-defined-wins-Regel → `:hover` (grau) übermalte beim Hover. Fix KISS (5 LOC, Trivial-Klausel laut CLAUDE.md): Reihenfolge umgedreht (`:hover` VOR `:checked`) + zusätzlicher `:checked:hover` Selektor mit identischem Style zu `:checked` (höhere Spezifität durch Selektor-Kombination). Andere Buttons unbeeinflusst. 3 Regressions-Tests `test_p96_power_btn_color.py`. Tests 1658→1661 (+3). Field-Test ohne Radio. **Vorgänger v0.97.67 P95 Bundle:** TUNE-Rechtsklick + QSO-Spalten-Config. Mike-Wunsch 20.05. nach P94: 2 UX-Verbesserungen. **Feature A:** Rechtsklick auf TUNE-Button → Menü 10s/15s/20s → TUNE mit dieser Dauer, Setting `tune_duration_s` UNCHANGED. Bei laufender TUNE: stop, kein Auto-Restart. **Feature B:** Rechtsklick im QSO-Fenster → Toggle für Even/Odd-Tag + Antennen-Anzeige mit speichern/laden (analog RX-Panel). Wirkt auf BESTEHENDE Einträge (Re-Render via neue `_entries`-SOT-Liste). Architektur: `_RadioCard` mit `tune_override_requested`-Signal + Kontextmenü, ControlPanel reemittet. `_on_tune_clicked` → `_tune_start(duration_s)` Helper extrahiert (Hardware-Sicherheit 10W FEST + ANT1 für beide Pfade identisch). QSO-Panel: `_block_timestamps` → `_entries: list[dict]` als SOT, 6 add_*-Methoden refactored, `_rerender_all` mit absoluter Scroll-Restore (R1-F2), `_on_log_context_menu` mit Standard-Aktionen ownership-sicher via `setParent(None)` (R1-F1). Settings-Persistierung 2 Keys (`qso_show_eo_tag`, `qso_show_ant_label`) mit try/except. V1 → V2-Self-Review (8 Findings) → R1-DeepSeek (2 Findings: F1 Ownership + F2 Scroll-Absolute) → V3+Code+Tests → Final-R1 V4-pro „Push-Freigabe: Ja" 0 Blocker. **V4-pro 42-Cycle-Bilanz: 0 Halluzinationen, 2 ORANGE gefangen.** Tests 1638→1658 (+20 P95). Field-Test F1-F7 pending (alle ohne Radio testbar). **Vorgänger v0.97.66 P94** Quick-73-Ignore. Mike-Field-Test 20.05. nach P93 (Screenshot 9A4AA): QSO komplett (RR73+73+courtesy), 4 Min später ruft 9A4AA erneut mit Report → App startet vollen Report-Austausch erneut → Pile-Up + chaotisches Panel. ADIF-Dedup-Filter (300s) verhinderte nur den Logbuch-Eintrag, nicht den Funkverkehr. **Mike-Spec:** Innerhalb 30 Min nach QSO-Ende, gleiche Station ruft mit Report/Grid → einmal `<their> <my> 73` senden (höflich) → für 30 Min komplett ignorieren (auch keine weitere 73) → State-Machine bleibt IDLE/CQ_WAIT (kein neues QSO). Fix KISS Variante A (DeepSeek-empfohlen): Pre-Filter `_p94_quick73_filter` in `ui/mw_cycle.py` VOR OMNI-Block und `qso_sm.on_message_received`. Konstante `_QUICK73_WINDOW_S = 1800`. Neuer Set `_quick73_sent` in MainWindow. Wiederverwendung von `_recent_logged_calls` als Datenquelle. Plus Auto-Hunt-Konsistenz: `core/auto_hunt.py:_RECENT_QSO_COOLDOWN_S` 300→1800 (Hard-Cap 10 Min UNCHANGED — Bot-Tarn-Schutz wahren). V1 (vor Compact) → V2-Self-Review (12 V1-Findings korrigiert: Datei-Pfad mw_qso→mw_cycle, Methoden-Name, Filter-Position vor OMNI, State-Check, audio_freq_hz, etc.) → R1-DeepSeek (1 🟠: audio_freq_hz-Restore via tx_finished-Signal) → V3+Code+Tests → Final-R1 V4-pro „PUSH FREIGEBEN ✅" 0 Blocker. **V4-pro 41-Cycle-Bilanz: 0 Halluzinationen, 1 ORANGE gefangen.** Tests 1626→1638 (+12 P94). Field-Test F1-F5 mit Radio pending. **Vorgänger v0.97.65 P93** Sende-Log-Eintrag auf Slot-Ende defern. Mike-Beobachtung 20.05. nach P73-A-Field-Test: „Sende"-Einträge erschienen am Slot-Start, „Empf."-Einträge am Slot-Ende → optisch „2 Meldungen gleichzeitig, dann 30 s Pause". Fix KISS: `_on_tx_started` speichert TX-Args in `_pending_tx_log`, `_on_tx_finished` ruft `add_tx` damit aus. Andere TX-Logik (`_has_sent_cq`-Flag) unverändert. Funkverhalten unverändert — reine UI-Anzeige-Verschiebung. Tests 1621→1626 (+5 P93). Mike-Direktive „minimaler Eingriff, eben" → kein voller V1→V2→R1-Workflow, aber Tests sichern Verhalten. Field-Test mit Radio pending. **Vorgänger v0.97.64 P73-A** Settings-UX TUNE-Tab konsolidiert. **P92+P91+P90+P89+P88+P86 Field-Tests ✅** durch Mike 19./20.05. Mike-Wunsch 18.05.: TUNE-Settings waren auf 2 Tabs verteilt → User-Sprung. Fix: neue `QGroupBox` „TUNE-Einstellungen" in Tab „TX & Schutz" mit 4 TUNE-Settings (Tuner-CB, TUNE-Dauer, Tune-Leistung, Auto-TUNE-Bandwechsel). Bonus R1-F1: Tuner-Checkbox als Master-Switch — bei Uncheck werden 3 abhängige Widgets disabled (UX-Hinweis). Settings-Keys + Save/Load-Pfade unverändert (rückwärtskompatibel). Final-R1 0 🔴 Bugs, 0 🟠 Risiken. **V4-pro 40-Cycle-Bilanz: 0 Halluzinationen.** Tests 1614→1621 (+7 P73-A). Field-Test F1-F4 pending (alle ohne Radio). **Vorgänger v0.97.63 P76-B** Auto-TUNE-Dauer-Anzeige 2-Phasen-Label. **P92+P91+P90+P89+P88+P86 Field-Tests ✅** durch Mike 19./20.05. Mike-Field-Test 18.05.: TUNE-Dauer „länger als 5 s" weil Soll-Anzeige `N / 5 s` während Phase B (Closed-Loop auf 10 W) + Post-Check weiterlief. Fix Variante B+ Hybrid (DeepSeek-empfohlen): Phase 1 = Soll-Anzeige unverändert + #AAA grau, Phase 2 (elapsed > duration_s) = „Leistung wird auf 10 W eingeregelt · X s" + #DDA heller Akzent. Defensive `max(1, duration_s)` Edge-Case. SWR/FWDPWR-Live-Werte in beiden Phasen sichtbar. Andere Pfade (Erfolg/Fehler/Cancel/Backup-Timeout) unverändert. Final-R1 0 🔴 Bugs „logisch korrekt, stabil, rückwirkungsfrei". **V4-pro 39-Cycle-Bilanz: 0 Halluzinationen.** Tests 1609→1614 (+5 P76-B). Field-Test F1-F3 pending (Bandwechsel mit Radio). **Vorgänger v0.97.62 P92** Diversity-Sub-Toggle bei Bandpilot=AN. **P92+P91+P90+P89+P88+P86 Field-Tests ✅** durch Mike 19./20.05. Mike-UX-Wunsch: 2. Klick auf DIVERSITY soll Std↔DX immer toggeln (auch bei bp=auto/manual). Bandpilot ist Empfehlung, kein Zwang. Heute war Sub-Toggle bei bp!=off gesperrt → Umweg DX→NORMAL→DIVERSITY→Wahl-Dialog→STANDARD nötig. Fix: Block-Klausel `if bp_mode != "off": return` in `ui/mw_radio.py:879+` raus + Tooltip-Hinweis „(nur bei Bandpilot=Aus)" in `ui/control_panel.py:1978` raus. Override-Lifetime gilt bis nächster Bandwechsel — `_on_band_changed → _maybe_apply_bandpilot` ist stateless, übernimmt automatisch wieder wenn bp!=off + Empfehlung vorliegt; ohne Empfehlung bleibt manueller Modus aktiv. R1 Final-Codereview 0 🔴 Bugs. **V4-pro 38-Cycle-Bilanz: 0 Halluzinationen.** Tests 1596→1609 (+13 P92, parametrisiert). Field-Test F1-F5 pending (Radio AN nötig). **Vorgänger v0.97.61 P91** Dialog-Lifecycle-Crash-Fix. **P91+P90+P89+P88+P86 Field-Tests ✅** durch Mike 19./20.05.

**ALT (v0.97.57 P87):** DX-Mode Warmup-Anzeige analog P85. Mike-Field-Test 19.05. DX gerade aktiviert: Ratio 50:50, aber Counts 26:06 (ANT1 dominiert) → Widerspruch wegen Pattern-Versatz. Fix KISS Variante A: `_dx_warmup_count: int` parallel zu `_win_rate_history`, < 4 → „Diversity läuft...", ≥ 4 → echte weak-counts. Reset-Lifecycle automatisch über `reset_win_rate_history`. Brainstorm-R1 Variante A 🟢, V2 0 Findings, Final-R1 „PUSH FREIGEBEN ✅" 0 KP. **V4-pro 33-Cycle-Bilanz: 0 Halluzinationen.** Tests 1563→1568 (+5 P87, 1 P85-T9 angepasst). Field-Test F1-F4 pending. **Vorgänger v0.97.56 P86** KALIBRIEREN Diversity-only.

**ALT (v0.97.56 P86):** KALIBRIEREN-Button nur in Diversity sichtbar. Mike-Field-Test P83-F1: UX-Verwirrung weil Normal-Mode-Kalibrierung nur Phase 2 (Gain) ohne Phase 3 (Ratio) machte → Inkonsistenz. Mike-KISS-Spec: KALIBRIEREN ist Diversity-only Feature (analog OMNI/Auto-Hunt). Fix: `_update_button_visibility` versteckt btn_einmessen im Normal. `_format_gain_status` ergänzt „→ DIVERSITY"-Hinweis im Normal-Mode-Stale. `_handle_dx_tuning` Defensive Early-Return. 30-Tage-Dialog erwähnt DIVERSITY. Brainstorm-R1 V4-pro Variante A 🟢, V2 fand 1 GELB (test_kalibrieren_normal_only_phase2 umgeschrieben), Final-R1 „PUSH FREIGEBEN ✅". **V4-pro 32-Cycle-Bilanz: 0 Halluzinationen.** Tests 1555→1563 (+8). Field-Test F1-F6 pending. **Vorgänger v0.97.55 P82** Hardware-Sicherheit Race-Fix.

**ALT (v0.97.55 P82):** „ohne Radio weiter" muss Connect IMMER überspringen (Hardware-Sicherheit). Mike-Field-Test 120 km vom Radio bei Radio AN → Race-Condition: Connect-Worker schneller als Klick → App startete TROTZDEM verbunden, ungewollter TX möglich via Auto-Hunt/CQ/TUNE. Fix 3 Schichten KISS: (1) ConnectStatusDialog `_user_cancelled`-Flag + `was_cancelled` Property gesetzt VOR `reject()`. (2) `_start_radio` Cleanup-Block: bei Cancel `_demo_mode_forced=True` + `abort_reconnect` + `radio.disconnect` falls `_running`. (3) Slot-Guards in `_on_radio_connected` und `_on_radio_disconnected`: bei `_demo_mode_forced` SOFORT raus (KEIN Hardware-Setup, KEIN Reconnect-Loop). V2 fand F1 ROT (`auto_connect` prüft kein Abort-Flag → Slot-Guards nötig statt nur Disconnect). R1-V4-pro 0 ROT 2 GELB (Race-Test T4 eingebaut). Final-R1 1 NACHBESSERN (try/except Konsistenz, behoben). Round 2 „Du kannst pushen". **V4-pro 31-Cycle-Bilanz: 0 Halluzinationen.** Tests 1548→1555 (+7 P82). Field-Test F1-F5 pending (alle ohne Radio testbar). **Vorgänger v0.97.54 Bundle M** Gain-Status + Win-%.

**ALT (v0.97.54 Bundle M):** P83 Gain-Status-Zeile mit Verfalls-Counter („noch X Stunden", farbcodiert grün/orange/rot, aktionsgetrieben KEIN Timer, auch im Normal-Modus) + P85 ANT2-Win-% statt Stationsanzahl (Median 4 Zyklen, löst 1-Zyklus-Versatz-Inkonsistenz „3:12+50:50"). R1-V4-pro Pre-Code 8 Findings, Final-R1 0 KP. Tests 1548. **V4-pro 30-Cycle-Bilanz: 0 Halluzinationen.**
