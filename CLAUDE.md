...
**Aktueller Stand:** v0.99.19 (05.06.2026) — **Optimierungs-Kampagne läuft autonom; Robustheits-Stufe (OPT-50..60) UND KISS-Stufe (OPT-61/63/64, OPT-62 obsolet) ABGESCHLOSSEN** (außer Mike-gesperrt OPT-55/58/59). Mike-Auftrag KISS/Robustheit > Speed, NUR Optimierungen, keine Verhaltensänderung. Erledigt, je DeepSeek-R1 **FREIGEBEN**, **ANT1=TX unberührt**, Tests durchgehend grün (jetzt **2469**): (z1) **OPT-64 `_valid_bands`-Modul-Funktion v0.99.19** (KISS, voller Workflow, LETZTER KISS-Punkt): `get_enabled_bands`+`set_enabled_bands` (`config/settings.py`) teilten dieselbe Band-Validierung (Filter auf BAND_FREQUENCIES-Strings, dedup mit Reihenfolge-Erhalt, Fallback Default) → Modul-Funktion `_valid_bands(raw)` (DeepSeek-R1: Modul-Fn statt Instanz-Methode, kein self-State). Variante A: get exakt gleich, `set(None)`→Default statt Crash (Robustheit, im Betrieb nie ausgelöst). R1 GO + Final-R1 PUSH FREIGEBEN; 2461→2469 (+8 `test_valid_bands.py`). Commit `c9726b7`. (z0) **OPT-63 `_resolve_station_position`-Helfer v0.99.18** (KISS, voller Workflow): Locator-Auflösung-Duplikat in `direction_map_widget.py` (`snapshot_to_station_points` + `entries_to_station_points`) → Modul-Helfer (DB-Lookup mit prec_km → Fallback-Locator → `safe_locator_to_latlon` → prec 5/110) → `(lat,lon,prec_km,loc)|None`; Fallback-Quelle + is_mobile/Rescue/StationPoint-Bau bleiben im Aufrufer, `cache.update` nur bei Fallback-Treffer (Guard `is not None`). Reine Dedup, kein TX-Pfad. DeepSeek R1 GO (Verhaltensgleichheit a/b/c verifiziert) + Final-R1 PUSH FREIGEBEN; 2453→2461 (+8 `test_resolve_station_position.py`). Commit `7f0b0e3`. (z) **OPT-61 `is_busy`-Property v0.99.17** (KISS-Stufe, voller Workflow): neue `@property is_busy` in `QSOStateMachine` (`core/qso_state.py`, Inline-Tupel `not in (IDLE,TIMEOUT,CQ_CALLING,CQ_WAIT)`) ersetzt dasselbe 4-State-Set an **7** UI-Call-Sites (main_window 3× — `_qso_active_for_msg_defer`/`_in_qso`/CQ-Stop-Guard; mw_cycle 2×; mw_qso 2×): 6× `not in`→`is_busy`, 1× `in`→`not is_busy`. **⭐ verify-don't-assume: Audit sagte „11×", Live-grep zeigte real genau 7** — übrige CQ_WAIT-Treffer sind ANDERE Mengen (2-State `(IDLE,CQ_WAIT)`, 3-State `(IDLE,CQ_WAIT,CQ_CALLING)`, 6-State mit WAIT_73/TX_73_COURTESY) → bewusst unangetastet. 2 tote lokale `QSOState`-Importe (main_window) mit raus (pyflakes-sauber, kein undefined). Reine Dedup, keine Verhaltensänderung. DeepSeek R1 GO (6 Fragen, 0 Korrekturen) + Final-R1 **PUSH FREIGEBEN**. Test-Fix `test_p81` (T1 mockte `qso_sm`→`is_busy` truthy; Helper `_qso_sm_in_state` baut echte SM, keine Logik-Dup; T2 jetzt aus richtigem Grund grün); 2438→**2453** (+15 `test_qso_is_busy.py`: 12 Enum-States + Komplement-Mutationsbeweis). Commit `6a48ea6`. (a) **Stufe 1 toter Code v0.99.10** (Bundles A–D: tote Methoden/Signale/Stubs · ungenutzte Imports 17 Dateien · tote Locals 10 · f-Strings 15; 2426→2425); (b) **3 tote Module entfernt v0.99.11** (OPT-Q1/Q2 — Mike-Entscheid „nicht FT2-relevant → weg": `ap_decoder`/`osd_decoder`/`diversity_merger` + Test; **diversity_merger ≠ Slice-B-Reserve**, flexradio-Slice-B bleibt; 2425→2415); (c) **OPT-50 Start-Crash-Schutz v0.99.12** (settings-Migration-`save()` in `try/except`; **OPT-51 war schon da**; 2415→2416 +Mutationstest); (d) **OPT-54 `atomic_write_json`-Helfer v0.99.13** (Stufe 2 Robustheit): neues `core/atomic_json.py` (DRY); `ntp_time._save_current` von nicht-atomarem `write_text` umgestellt → **Atomaritäts-Lücke geschlossen** (Crash mid-write konnte `dt_corrections.json` zerreißen) + 5 core-Stores migriert (`awards_prefs`/`rf_preset_store`/`mode_recommender`/`psk_reporter`/`locator_db`); 3× totes `import os` raus; `dump_kwargs` durchgereicht → Bytes **bit-identisch**. **`settings.py` bewusst NICHT migriert** — `core/__init__` lädt decoder+encoder+ft8_lib, ein `from core…`-Import zöge dieses schwere Paket in jeden isolierten settings-Import; settings ist eh schon atomar (DeepSeek Final-R1 segnete die Scope-Korrektur ab); 2416→2424 (+8 `test_atomic_json.py`)); (e) **OPT-53 Settings-Typvalidierung v0.99.14** (Stufe 2 Robustheit): `config/settings.py:load()` übernahm config.json-Werte **blind** (`update(saved)`) → neue `_validate_types()` prüft jedes DEFAULTS-Feld gegen `type(value) is type(default)` und resettet bei Mismatch auf den Default; **`type() is type()` statt isinstance** = bool/int sauber getrennt (`isinstance(True,int)`==True → ein versehentliches `"flexradio_port": true` würde sonst durchrutschen; Mutationsbeweis-Test); iteriert **nur über DEFAULTS** → dynamische Keys (`enabled_bands`/`tx_slot_lock`/Presets) unberührt; Normalbetrieb 0 Änderung; 2424→2433 +9); (f) **OPT-56 closeEvent-except (v0.99.15)** + **OPT-57 station_stats Thread-Stop (v0.99.16)** + **OPT-60 geprüft** (Stufe 2 Robustheit, je voller Workflow): OPT-56 = breites `except Exception: pass` um `audio_monitor.stop()` raus → `getattr`-Guard (stop() ist intern robust/idempotent; das äußere except verschluckte echte Bugs), Audit-Teil dx_tuning = NICHT-FUND (`_dx_tune_dialog` hat `parent=self` → Qt schließt ihn; Laufzeit-State, kein TX-Pfad); OPT-57 = `station_stats`-Daemon-Writer bekam sauberen Stop via Modul-**Sentinel** `_SHUTDOWN` + `shutdown(timeout=5)` (FIFO-Drain-Garantie statt Audit-„Event"; closeEvent ruft's nach timer/decoder-stop), Laufbetrieb 0 Änderung; OPT-60 = ~60 stille `except: pass` triagiert → **kein systematischer Handlungsbedarf** (verify-don't-assume: **0 bare `except:`**, 15 im gesperrten flexradio-TX-Pfad, Rest legitimes Best-Effort → Massenumstellung wäre Busy-Work, herabgestuft auf opportunistische Einzelfixes); 2433→**2438** (+5: test_audio_monitor +1, test_stats_shutdown +4). Plan/Tracker **`OPTIMIERUNGSWORKFLOW.md`** (maßgeblich), Rückfall-Tag `v0.99.9-pre-optimierung`. **OPT-62 geprüft = OBSOLET** (verify-don't-assume + DeepSeek-R1: K2-Befund durch P80/P51-Migration + Bundle A überholt — Preset-Methoden nur noch test-only/tot/no-op; `save_*`-Entfernung = Mike-Vorlage OPT-Q4, Claude-Empfehlung BEHALTEN). **▶ KISS-Stufe KOMPLETT. Verbleibende Stufen (Mike: Speed NACHRANGIG → Richtungs-Abstimmung empfohlen): Speed (OPT-05..11/20/23/24, Decoder-nah) + große Methoden (OPT-65/66/30..32, langfristig).** **⛔ NUR nach Mike-Rückmeldung: TX-Pfad-Verdacht OPT-59 + Bug-Verdachte OPT-55/58, Threading OPT-52, OPT-Q4 (save_*-API).** **✅ GEPUSHT (Mike-Freigabe 05.06.2026): 25 Commits v0.99.10–v0.99.19 auf origin/main `e097acc`, 0 ungepusht.** — **Vorgänger v0.99.9: Diagnose-Logging für ungeklärten Watt-/RF-Bug (verhaltensneutral)** (Diagnose-Feature, DeepSeek-R1 Design + Final-R1 Diff = verhaltensneutral bestätigt). **Mike-Field:** sporadisch zeigt die App **RF 100 %**, das Funkgerät macht aber nur ~60 W (Ziel 70 W) und die Regelung „klebt" (passt nichts mehr an); heilt sich beim Umschalten der Watt-Zahl (frischer `set_power`). SWR durchgehend 1,3 (kein Foldback). **⚠️ Erste Diagnose war FALSCH — Mike hat sie widerlegt (verify-don't-assume):** ich tippte auf „Hardware-Decke/Clipschutz-Limit"; Mikes 2. Screenshot zeigte Ziel **80 W → RF 80 % → 83 W** (rfpower NICHT am Anschlag) → rfpower↔Watt ist normal ~linear, die 100 %/60 W sind ein **echter Sync-/Freeze-Fehler, kein Limit**; auch der **Clipschutz (75 %) ist nicht die Ursache** (bei 75 % Audio kommen 83 W → Audio-Drive nicht der Flaschenhals). **Leitende Hypothese:** `_auto_adjust_tx_level` schickt keine neuen `set_power` mehr, sobald `new_rfpower==_rfpower_current` (Anschlag) → markiert konvergiert + verstummt; wenn das Radio seine Leistung zwischendurch zurücksetzt (Bandwechsel ODER Diversity-Slice-/Antennen-Umschaltung laden ein Profil — `set_power`-Kommentar warnt davor), bleibt die App auf 100 %, das Radio tiefer → klebt; Watt-Knopf = frischer `set_power` = Heilung. **Mike-Plan: erst diagnostizieren statt raten-fixen.** **Gemacht (reines Logging, KEINE Regelungsänderung, ANT1=TX unberührt):** `ui/mw_tx.py` `debug_log("TXPWR", …)` an 3 Stellen — `_auto_adjust_tx_level` (1×/Sende-Slot: {band/mode, target, app_rf%, fwdpwr, audio, peak, swr, conv-Flags, action ∈ set_rf->N/converge_save/hold}; **DeepSeek-R1-Catch:** `action="hold"`-Default VOR if/elif gegen NameError im In-Band-Zweig), `_on_power_changed` (Watt-Knopf = die „Heilung"), `_apply_rf_preset` (Band-/Watt-/Init-Wechsel laden rfpower neu, hit/default). **Nur bei aktivem Debug-Log (Ctrl+D), 1×/Slot → kein Hot-Path-Flood** (bewusst NICHT in den FWDPWR-Meter-Callback — das wäre die 4-GB-Falle von v0.99.3); mit bestehenden `[ANT]`-Logs per Zeitstempel korrelierbar (Diversity-Switch → danach kleben?). DeepSeeks Zusatz-Log am Band-/Diversity-Switch **bewusst verworfen** (redundant: Band via `_apply_rf_preset`-Log, Diversity via `[ANT]`-Logs — KISS). DeepSeek Final-R1: verhaltensneutral, `action` in allen Pfaden definiert, kein Flood. **Bug bleibt OFFEN** (TODO.md) — bei Wiederauftreten Debug-Log an → `~/.simpleft8/debug_*.log` nach `[TXPWR]` lesen → echte Ursache → gezielter Fix. Tests 2423→**2426** (+3 `test_txpwr_diag.py`: `action`-NameError-Mutationsbeweis, rfpower-Hoch, Konvergenz-Speicher-Pfad). **NICHT gepusht (19 Commits seit Anker `bfa20dd`).** — **Vorgänger v0.99.8 (05.06.2026) Einmess-Fenster verschlankt (ein Fortschritts-Zähler statt drei, doppelter Titel raus)** (voller Workflow, DeepSeek Design-R1 [Aufbau GO] + Final-R1 PUSH FREIGEBEN [0 Blocker]). **Mike-Field:** das Diversity-/Gain-Kalibrier-Fenster (`ui/dx_tune_dialog.py`) war zu groß und zeigte **drei/vier Fortschritts-Angaben gleichzeitig** („Runde 1/2", „Schritt 5/12 (5/6 in dieser Runde)", Balken „4/12 Zyklen") + einen **doppelten Titel** (Titelleiste + großes Body-Label) — verwirrend (die 4 und die 5 widersprachen sich: Balken=`_step`, Label=`_step+1`). **Umbau (reine Anzeige-Logik, Mess-/Scoring-/Antennen-Logik UNBERÜHRT, TX bleibt ANT1):** großes Body-Titel-Label raus (`_title_label` war nirgends sonst genutzt; Fenstertitel `windowTitle` von `_get_mode_label()` bleibt — 4 Smoke-Tests asserten ihn); `step_label` zeigt nur noch das Lebenszeichen **„Gerade: ANT2 · 10 dB"**; der Balken-Text trägt Zyklus + Restzeit zusammen **„Zyklus 5 / 12 · noch ~2:00 min"** (in `_update_time` via `progress.setFormat`); **Off-by-one behoben** — Balkenwert zählt den LAUFENDEN Zyklus mit (`setValue(_step+1)`) → Balken und Text nennen dieselbe Zahl. **`time_label`-Widget entfernt** (Restzeit wandert in den Balken); **`mode_label` „Misst gleichzeitig für Standard- und DX-Modus" entfernt** (+ aus `_apply_state_ui`-Visibility-Schleife); **gelber 2-Zeilen-Fachjargon-Block** („12 Zyklen interleaved …") → **eine graue Zeile** „TX bleibt auf ANT1 · vergleicht ANT1 ↔ ANT2" (↔ statt seltenem ⇄, DeepSeek-Font-Hinweis); **`detail_label`-Widget BLEIBT** für ADC-Übersteuerungs-Warnung + Abschluss-Meldung (Normalbetrieb leer); Results-Header „(Top-5 SNR-Schnitt …)" → „— Ø SNR pro Antenne & Gain"; **Höhe 460/490/510 → 360/390/410 px**; veraltete „8 Schritte"-Kommentare → „12 Schritte" (Schedule real 12: `GAIN_VALUES=[0,10,20]`×2 Antennen×2 Runden). DeepSeek Design-R1 segnete den Aufbau ab (3 Verfeinerungen übernommen: „Runde 1/2" raus, Tabellen-Kopfzeile, „gilt für Std+DX" weg; Modal-Annahme von DeepSeek war falsch [Dialog ist `setModal(False)`], ohne Folge) + Final-R1 **PUSH FREIGEBEN** (kein toter Verweis, Balken-Logik über alle 12 Schritte + Adaptiv-Stop konsistent, keine Regression). Tests 2417→**2423** (+6 `test_calibration_dialog_smoke.py`; P146-mode_label-Test auf „entfernt" umgeschrieben). **NICHT gepusht (14 Commits seit Anker `bfa20dd`), Field-Test pending (visueller Check: passt alles in 360 px ohne Abschneidung?).** — **Vorgänger v0.99.7 (04.06.2026) Auto-Hunt aus akkumuliertem Pool + Frische-Fenster + „gearbeitete auch anrufen"-Schalter** (voller autonomer Workflow, DeepSeek Plan-R1 GO [3 Korrekturen] + Final-R1 PUSH FREIGEBEN [7 Prüfpunkte, 0 Blocker]). **Mike-Field (vor Compact abgestimmt), zwei Probleme aus einem Guss:** (1) Auto-Hunt sah nur den **Moment-Slot** (`mw_cycle._run_auto_hunt(messages)` = 1 Decode-Zyklus), während die RX-Liste über `station_accumulator` akkumuliert (CQ-Rufer bis `AGING_SLOTS_CQ_CALLER=20` Slots) → man SIEHT eine 45s-alte CQ-Station, Auto-Hunt ignoriert sie; bei FT4 schlimmer; bei leerem Decode-Slot wählte er gar nichts. (2) Kein Schalter um schon gearbeitete Stationen (P169-Worked-Filter, Band+Mode-genau) für die Diplom-Jagd erneut anzurufen. **Lösung (KISS, additiv, `select_next`-Signatur unverändert):** **Pool statt Moment-Slot** — neue Modul-Konstante `core/auto_hunt.py:AUTO_HUNT_FRESH_SLOTS={"FT8":3,"FT4":3,"FT2":3}` + Helper `mw_cycle._build_auto_hunt_pool()` baut die frischen CQ-Rufer aus `_diversity_stations`: `is_cq` (**Live-Property** aus field1 → ins QSO gewechselte Station fällt automatisch raus) UND `(now − _last_heard) ≤ fresh·slot + 1.0` (P157 setzt `_last_heard` immer; **+1,0 s Jitter-Puffer** DeepSeek). `_run_auto_hunt` übergibt diesen Pool an `select_next` — Pool ist aktuell (`accumulate_stations` läuft VOR `_run_auto_hunt`). **Hauptgewinn:** Auto-Hunt wählt jetzt auch bei leerem Moment-Slot aus dem Pool. **„3" begründet:** eine CQ-Station ruft modus-invariant jeden 2. Slot → 3 Slots = überall ≤1 verpasster Ruf; FT4 ruft HÄUFIGER, fängt man schneller → FT4 nur datenbasiert auf 4, NICHT auf Verdacht. **„gearbeitete auch anrufen"-Schalter** — Instanz-Flag `_skip_worked=True` (Default) + Setter `set_skip_worked`; Worked-Filter-Block in `select_next` (inkl. `n_before_worked`+`all_worked`-Emit) nur unter `if self._skip_worked and self._qso_log is not None:` (DeepSeek-Korrektur: geschlossener Block, kein NameError). Setting `config/settings.py:auto_hunt_call_worked` (Default `False`) + Checkbox „Schon gearbeitete Stationen auch anrufen" (`settings_dialog`, Tab „FT8 & Diversity") + load/save/reset; `_run_auto_hunt` setzt das Flag **pro Slot live** → Settings-Änderung wirkt sofort, kein Dialog-Hook. **Steuert NUR Auto-Hunt — der NEUE-Filter der RX-Liste bleibt getrennt.** **Klarere Meldung** `_on_auto_hunt_all_worked`: „alle N aktiven CQ-Rufer auf {Band} {Mode} schon gearbeitet" (N = Pool-Größe = was man sieht; im Diplom-Modus feuert sie bewusst nie). DeepSeek Plan-R1 GO (3 Korrekturen eingebaut: +1,0 s statt halber Slot; Worked-Block klammern; Fallback-Pool als Kommentar statt Log-Zeile [Normal-Pfad tot, Import wäre Overengineering]) + Final-R1 **PUSH FREIGEBEN** (Worked-Block geschlossen, Frische defensiv, kein Race, Doppel-Pick via `_recent_qso`-Cooldown ausgeschlossen, Hardware unberührt). **Reine State-/Auswahl-/Anzeige-Logik, kein TX-Pfad-Eingriff, ANT1/ANT2 unberührt** (Auto-Hunt-TX läuft unverändert über `start_qso`). Tests 2405→**2417** (+12 `test_autohunt_pool.py`; `test_p123` T2-Mock um `_build_auto_hunt_pool`-Stub ergänzt). FEATURES §25 (neu). **✅ FIELD-VALIDIERT (Mike 05.06.2026): „es werden jetzt auch ältere Stationen gerufen wenn das in den Einstellungen gewollt ist" — Pool-Auswahl greift, Diplom-Schalter steuert die gearbeiteten. NICHT gepusht.** — **Vorgänger v0.99.6 (04.06.2026) HALT→STOPP: ein zentraler Notstopp für alles** (voller Workflow, DeepSeek Plan-R1 [PLAN ÜBERARBEITEN → TUNE-Träger-Lücke ergänzt] + Final-R1 PUSH FREIGEBEN). **Mike-Field (akut, sicherheitsrelevant):** Auto-Hunt + OMNI ließen sich nicht stoppen — (A) STOPP-Knopf war **ausgegraut** bei Auto-Hunt/OMNI im IDLE-Zwischenzustand + während Diversity-Messung (Enable-Logik kannte nur „QSO/Normal-CQ") → kein Notaus, Catch-22 „erst HALT drücken"; (B) Modus-Toggle-OFF armierte bei laufendem QSO nur, statt zu stoppen. Mike: „HALT heißt Notstopp." Button **„HALT"→„STOPP"** + Tooltip „Alles wird sofort abgebrochen". STOPP-Knopf + Auto-Hunt-Toggle + OMNI-Toggle rufen alle dasselbe **`_execute_full_halt()`** — kompromisslos sofort, kein Armieren. Das ist jetzt das Notstopp-Modul für JEDE TX-Quelle: Encoder-TX, CQ, QSO, OMNI, Auto-Hunt + **NEU (DeepSeek-Plan-R1 🔴):** **TUNE-Träger** (`_tune_stop(None)` wenn `_tune_active` — der lief über einen eigenen Weg, NICHT `_abort_active_tx`; bei aktivem TUNE blieb der Träger trotz STOPP an = Sicherheitsverstoß „TX jederzeit beenden") + Einmess-Dialog (`_dx_tune_dialog.reject()`) + Diversity-Gain-Mess-Lock (`_set_gain_measure_lock(False)`). **STOPP-Knopf IMMER drückbar** (`btn_cancel.setEnabled` aus `_on_state_changed` + beiden mw_radio-Lock-Methoden `_set_cq_locked`/`_set_gain_measure_lock` entfernt). Entfernt: `_arm_deferred_halt`/`disable_cq_resume`/`QSO_IN_EXCHANGE_STATES`/`set_halt_armed`/`_halt_armed_style`/`_halt_armed`/IDLE-Armier-Block; `cancel()` setzt `_was_cq=False` (DeepSeek 🟡). **Reine State-/UI-Logik, kein TX-Antennen-Eingriff (ANT1=TX) — STOPP schaltet jetzt jeden Träger ab (Sicherheits-PLUS).** DeepSeek Plan-R1 fand die TUNE-Lücke (Plan überarbeitet) + Final-R1 **PUSH FREIGEBEN** (alle TX-Pfade abgedeckt, Reihenfolge sicher, idempotent, keine toten Reste). Tests 2402→**2405** (`test_halt_unified.py` neu geschrieben [14]: alle TX-Quellen einzeln + TUNE-nur-wenn-aktiv + Bug-A-Source-Regression „btn_cancel nie ausgegraut"; `test_p81` Text HALT→STOPP). FEATURES §23 (neu geschrieben). **NICHT gepusht, Field-Test pending.** — **Vorgänger v0.99.5 (04.06.2026) WAIT_73-Horchphase nach RR73 von 3 auf 2 Slots verkürzt (Auto-Hunt-Pause ~60→45 s, WSJT-X-konform)** (voller Workflow, DeepSeek Plan-R1 [Off-by-one bestätigt, 0 Bugs] + Final-R1 PUSH FREIGEBEN [0 Bugs/0 Risiken]). Mike-Field: ~60 s Pause zwischen abgeschlossenem QSO und nächstem Auto-Hunt-Ruf. Nach RR73 ist das QSO bereits geloggt; `on_cycle_end` WAIT_73-Branch horchte **3 Leer-Slots (45 s)** auf ein 73, das im verkürzten FT8-Modus meist nie kommt — Auto-Hunt darf in der Zeit nichts picken (`qso_idle`-Guard). WSJT-X-Recherche + DeepSeek bestätigt: der **verkürzte** Modus (RR73) wartet NICHT auf ein 73 (nur der lange RRR-Modus), beobachtet genau **1 Empfangs-Slot** (Nachsende-Schutz). `core/qso_state.py`: neue Modul-Konstante **`WAIT_73_MAX_CYCLES = 2`** (war hartcodiert `3`); der WAIT_73-Branch nutzt sie. **„2" ist die Untergrenze:** `on_cycle_end` triggert am Slot-START → der erste (einzig relevante) RX-Slot wird noch voll abgewartet (73/R-Report via `on_message_received`); `1` würde davor triggern → 73 verpasst. Höflichkeits-73-Pfad + `wait_73_retries` + `_resume_cq_if_needed` + `MAX_QSO_DURATION`-Check **unverändert**. **Stufe 1 von 2:** Höflichkeits-73 bleibt als **Kompatibilitäts-Brücke** zu langen/IC-7300-Stationen (Stufe 2 „73 ganz raus" verworfen — 0 Zeitgewinn, mehr Risiko; **keine 2-Modi-Einstellung** — verkürzt ist protokoll-kompatibel zum langen Modus, Schalter wäre Overengineering). **Reine Timing-/State-Logik, kein TX-Antennen-Eingriff, ANT1/ANT2 unberührt.** Tests 2398→**2402** (+4 `test_wait73_threshold.py`: Konstante==2, kein Trigger vor Schwelle, Trigger genau bei Schwelle, Off-by-one-Garantie [73 im ersten RX-Slot gefangen — bräche bei Schwelle 1, Mutationsbeweis]; `test_p33` T2.3 + `test_p1_10` Test 8 konstantengerecht via `range(WAIT_73_MAX_CYCLES)`). FEATURES §24. Rückfallpunkt `bfa20dd` gepusht (`git checkout bfa20dd -- core/qso_state.py`). **NICHT gepusht, Field-Test pending.** — **Vorgänger v0.99.4 (04.06.2026) Einheitliche Bedienung über HALT + smartes HALT (Ruf sofort / QSO deferred)** (voller Workflow, DeepSeek Plan-R1 [2 Korrekturen] + Final-R1 6/6 PUSH FREIGEBEN). Mike-Wunsch: Bedien-Asymmetrie weg (Auto-Hunt→OMNI nur via HALT, umgekehrt direkt). Jetzt alles über HALT: Modus-Buttons (OMNI/Auto-Hunt) starten NUR aus Ruhe (`state in IDLE/CQ_WAIT` + anderer Modus inaktiv, sonst „erst HALT" — kein Supersede mehr); OFF→`_on_cancel()` mit `is_active()`-Re-Entry-Guard. **HALT smart:** Ruf (kein Rapport, bis `WAIT_REPORT`)→sofort; laufendes QSO im Austausch (ab `TX_REPORT`, `QSO_IN_EXCHANGE_STATES`)→armiert (QSO läuft+loggt zu Ende→IDLE; Info-Zeile + oranger „HALT •"); 2×HALT=sofort hart. `core/qso_state.py`: neue `disable_cq_resume()` (cq_mode+`_was_cq`+`_caller_queue` — reines stop_cq() ließ `_was_cq`/Queue stehen → `_resume_cq_if_needed` belebte CQ wieder, DeepSeek-R1) + Konstante `QSO_IN_EXCHANGE_STATES`. `mw_qso._on_cancel`=Dispatcher (armiert→`_execute_full_halt`/Austausch→`_arm_deferred_halt`/Ruf→full); `_arm_deferred_halt` stillt Auto-Hunt/OMNI/CQ-Resume+pending, bricht QSO NICHT ab; `_on_state_changed(IDLE)` hebt armiert auf. `control_panel.set_halt_armed()`. **Reines State-/UI-Verhalten, ANT1/ANT2 unberührt.** Tests 2387→**2398** (+11 `test_halt_unified.py`; 12 Bestands-Tests angepasst). NICHT gepusht, Field-Test pending. — **Vorgänger v0.99.3 PSK-Timer-Spin behoben (4 GB-Debug-Log-Flut) + OMNI-Diagnose-Marker** (DeepSeek R1 6/6 für Log-Fix). PSK: `_reset_psk_polling_on_change` startet `_psk_timer` mit `start(0)` (Intervall 0 = Event-Loop-Dauerfeuer); lag die Intervall-Umschaltung in `_fetch_psk_stats` HINTER dem `_has_sent_cq`-Return, spinnte der Timer endlos (4 GB/Tag + CPU) und schrieb die SKIP-Logzeile tausende Male/s. Fix: Umschaltung VOR den Return (Timer verlässt das 0-Intervall nach 1. Tick immer); Flut-Logzeile raus; ~6 GB Altlasten gelöscht (`debug_log` öffnet/schließt pro Write → gefahrlos). Retention (`cleanup_old_files keep_days=1`) bestand schon. OMNI: `debug_log("OMNI",…)`-Marker (START/STOP/PAUSE/RESUME + `on_cycle_start` paused/parity/TX/busy + `_maybe_resume_omni`) → nächster Auto-Hunt→OMNI-Wechsel zeigt im Log warum ~1:45 vergehen (verifiziert: `resume_after_qso` behält alte Parität = ≤1 Slot, erklärt NICHT die vollen 1:45 → Marker decken den Rest auf). Reines Timing/Logging, ANT1/ANT2 unberührt. Tests 2384→**2387** (+3 `test_psk_timer_no_spin.py`). NICHT gepusht. — **Vorgänger v0.99.2 DT-Kalibrier-Knopf (⏱) nur auf FT8 sichtbar** (FT4/FT2 ausblenden = Fehlklick-Schutz; voller Workflow, DeepSeek Final-R1 4/4). `ui/rx_panel.py:set_calibrate_visible(visible)`; `mw_radio._on_mode_changed` schaltet `mode=="FT8"` (zentraler Mode-Hook); `main_window` Initial-Sichtbarkeit; FT8-Guard in `_on_calibrate_dt` bleibt als Sicherheitsnetz. Reines Sichtbarkeits-Feature, ANT1/ANT2 unberührt. Tests 2380→**2384** (+4 `test_calibrate_button_ft8only.py`). NICHT gepusht. — **Vorgänger v0.99.1 Eingeklappter RADIO-Header: Netto-Watt + farbiges SWR beim Senden** (voller Workflow, DeepSeek Plan-R1 GO + Final-R1 PUSH FREIGEBEN). Die eingeklappte RADIO-Kachel zeigt beim Senden „— 80 → 58 W · SWR 1.2" (SWR farbig per Ampel grün/gelb/rot), im Empfang weiter „— 80 W". `ui/control_panel.py`: Modul-Helper **`swr_color(swr)`** (DRY — `update_swr` nutzt ihn jetzt auch); `_refresh_radio_status_label` hängt bei `_last_watt>0` (TX, gleicher Guard wie `_refresh_netto`) Netto-Watt (`compute_net_power`) + farbiges SWR via Rich-Text-`<span>` an (Label-Grundfarbe `#00aacc` bleibt für Watt-Teil); Live-Trigger in `update_watt`/`update_swr`/`reset_swr_display` (FWDPWR-Meter→~0 im RX → Header zurück auf „— {p} W"); `getattr`-Default für `_last_watt`/`_last_swr_for_netto` (Refresh läuft in `__init__` vor deren Init). **Reines Anzeige-Feature, kein TX-/Antennen-Eingriff, ANT1/ANT2 unberührt.** Tests 2370→**2380** (+10 `test_radio_header_collapsed.py`). **✅ v0.99.0 DT-Kalibrierung field-validiert (Mike am Radio):** „klappt super", DT sehr gut im ±-Bereich, FT4 gut, App flüssiger. NICHT gepusht. — **Vorgänger v0.99.0 DT-Korrektur: dynamisches Dauer-Lernen RAUS, manueller Kalibrier-Knopf REIN** (großer Umbau, voller Workflow: V1→V2→R1→V3→Code→Tests→Final-R1, beide DeepSeek-Runden PUSH FREIGEBEN). Die automatische DT-Lernschleife (`core/ntp_time.py`: Mess-/Operate-Phasen, Dämpfung, Sprung-Reset, Fast-Convergence) war über Tage fragil — der Wert pendelte, sprang ins Minus, ein FT8→FT4→FT8-Übergang (Decoder-Race) verdarb ihn → OMNI-CQ sendete 30/60s. **Kein fester Konstanten-Wert möglich:** Mikes Ferienhaus-iMac (2015, defekte Pufferbatterie, nicht dauerstrom) hat eine um 3–5s driftende System-Uhr → ein fester Wert veraltet, macOS-NTP greift erst verzögert/online. Lösung (Mikes Modell): der Wert ändert sich NUR auf Knopfdruck → stabil UND nachjustierbar. **`core/ntp_time.py`:** Lernschleife komplett entfernt (`_phase`/`_cycle_count`/`_measure_buffer`/`update_from_decoded`/Sprung-Reset/`DAMPING`/`*_MEASURE_CYCLES`/`OPERATE_CYCLES`/`DEADBAND`/Fast-Convergence/`reset()` raus); neu **`record_samples(dt_values)`** (puffert NUR — FT8-Slots in `deque(maxlen=RECENT_SLOTS=3)`, ~45s gleitendes Fenster; setzt Anzeige-Werte für alle Modi) + **`calibrate()→(ok,msg)`** (flacht das Fenster, `< MIN_STATIONS=5` → `(False,…)`; sonst MAD-Filter → Median der Residuen → **`_correction += median`** INKREMENTELL [die `m.dt` sind Residuen nach der aktuellen Korrektur → ein Klick konvergiert voll], voller Schritt OHNE Dämpfung, symmetr. Clamp ±1.0, **KEIN Negativ-Riegel** — vorlaufende Uhr darf negativ; `_save_current()`). `set_mode`/`set_band` leeren das Fenster. `get_time()` (reine FT8-Basis, Slot-Takt — v0.98.63-Fix intakt) + `get_correction()` (Basis + `_MODE_DELTA`, RX-Decode + Anzeige) + `_load_saved()`/Migration + MAD-Filter + `set_hardware_default(0.26)` unverändert. **UI:** ⏱-Knopf in `ui/rx_panel.py` (neben 🔊, Signal `calibrate_requested`), Handler `ui/mw_radio.py:_on_calibrate_dt` (nur FT8 — sonst Info „nur auf FT8"; KISS statt Ausgrauen; Ergebnis in QSO-Info-Zeile + Statusbar sofort). `ui/mw_cycle.py`: `update_from_decoded`→`record_samples`. Statusbar (`main_window.py`) ohne `_phase`. **Reines Timing/Anzeige, kein TX-Antennen-Eingriff, ANT1/ANT2 unberührt.** DeepSeek Plan-R1 GO (alle 10 Punkte: inkrementell bestätigt, voller Schritt, K=3, MIN=5, reset raus, Info-Meldung, Encoder-Robustheit als separates TODO) + Final-R1 **PUSH FREIGEBEN** (9 Prüfpunkte, 0 Blocker). Tests 2358→**2370** (+12, neu `test_dt_calibrate.py` [20]; Lern-/Phasen-/DEADBAND-/`reset()`-Tests durch Kalibrier-Äquivalente ersetzt; Versions-Asserts `test_p132`/`test_p134` auf Tupel-Vergleich). **Field-Fix gleiche Session:** mehrfaches Drücken ließ den Wert klettern (Puffer nach Kalibrierung nicht geleert → Doppel-Addition) → `_recent_samples.clear()` NUR im Erfolgsfall (DeepSeek R1 bestätigt). Encoder-Drift-Guard-Robustheit bleibt separates TODO (Schwung-Ursache mit stabilem Wert weg). Sicherheitsanker GitHub `22f3d07` (v0.98.63, alte DT-Berechnung). **⚠️ Mike: App NEU STARTEN** (kein Auto-Lernen — DT per ⏱ setzen). NICHT gepusht, Field-Test pending. — **Vorgänger v0.98.64 FT-Modus-Wechsel bricht laufendes QSO/TX ab (gemeinsamer Abbruch-Helper)** (voller Workflow): `_on_mode_changed` brach laufendes QSO+TX NICHT ab (Band- + RX-Mode-Wechsel taten es) → nach FT8→FT4-Klick sendete die QSO-Maschine LY7Z 3× weiter. Fix `ui/mw_radio.py`: identischen Abbruch-Block in Helper **`_abort_qso_and_tx()`** extrahiert, von ALLEN 3 Wechsel-Pfaden gerufen (Bug-Klasse strukturell weg) + Early-Return bei gleichem Modus. R1+Final-R1 PUSH FREIGEBEN. Tests 2349→2358 (+9). NICHT gepusht. — **Vorgänger v0.98.63 FT4-OMNI sendete 30s statt 15s — Slot-Takt vom Modus-Versatz entkoppelt** (voller Workflow, 2× DeepSeek). Regression aus v0.98.62. Mike-Field (OMNI-CQ FT4): TX nur alle 30s (Log je +30s), **intermittierend** (mal 15s mal 30s, je nachdem ob vorher FT8). Diagnose: Cycle-Timer (`timing.py:43`) leitet den Slot-Takt aus `ntp_time.get_time()` ab — und `get_time()` zog seit v0.98.62 den `_MODE_DELTA["FT4"]=−0.30` mit → `cycle_start` feuerte auf FT4 zu spät (an der Slot-Grenze statt davor) → OMNI-TX (`omni_cq.on_cycle_start`→`encoder.transmit`) landete im aktuellen Slot dessen Sende-Frist (Grenze−0.8) schon vorbei war → Encoder-Drift-Guard (`encoder.py:337`) +2 Slots, Folge-Slot „encoder busy" → 30s. **Schwellenabhängig:** kippt sobald `_correction<0.30` (FT4-effektiv ≤0 → Timer feuert an/nach Grenze); da nur FT8 misst und der Wert um ~0.27–0.45 schwankt, flackerte FT4-OMNI je nach FT8-Messwert. Fix (`core/ntp_time.py`, 1 Funktion): `get_time()` nutzt jetzt NUR die FT8-Basis `_correction` (OHNE `_MODE_DELTA`) → Slot-Takt immer deutlich positiv → deterministisch 15s. `get_correction()` (mit Delta) bleibt für RX-Decode-Shift (decoder:361) + Anzeige → FT4-Empfang/Anzeige zentriert (keine RX-Regression). Physikalisch korrekt (TX am echten Protokoll-Slot; −0.3-TX-Versatz würde uns bei der Gegenstation mit DT −0.3 zeigen). **Kein TX-Antennen-Eingriff, ANT1/ANT2 unberührt** (Encoder nutzt ohnehin reine `time.time()`). DeepSeek R1 (Diagnose+Fix wasserdicht, kein versteckter Pfad) + Final-R1 **PUSH FREIGEBEN** (Schwelle korrekt, Umsetzung exakt, keine Nebenwirkungen, Kaltstart-Edge bestätigt). Tests 2348→**2349** (`test_get_time_uses_effective_correction`→`test_get_time_uses_base_not_delta` umgedreht + neuer `test_slot_takt_invariant_but_rx_diverges`). **✅ Field-Test BESTANDEN (Mike am Radio 03.06.):** FT4-Sende-Takt korrekt (~8s Slot-Intervall, durch QSO bestätigt — kein 30s mehr), FT4-Empfang/DT-Zeiten sehr gut. — **Vorgänger v0.98.62 DT-Korrektur modus-abhängig (FT4-Versatz), ✅ field-validiert** (voller Workflow). FT8-DT um 0, FT4 alle ~−0.3 → der gelernte Wert ist modus-abhängig (FT8 ~+0.29, FT4 ~0; je schneller desto enger). `_MODE_DELTA={FT8:0,FT4:-0.30,FT2:0}` auf `get_correction()`; nur FT8 lernt Basis, FT4/FT2 erben+Delta. DeepSeek Final-R1 fand P171-Migrations-Bug (allnum-Fallback lud FT4=0.045 als falsche Basis ohne FT8-Keys) → gefixt (return bei fehlenden FT8-Keys). **✅ Feld-Test BESTANDEN:** FT4-DT −0.3→~0 (leicht +0.1), Empfang STABIL (11 Stationen, KEIN P168-Decode-Einbruch — Risiko entwarnt), FT4-QSOs liefen (LZ2II, SV7BAY). Offene Mikrojustierung (NICHT akut): optional `_MODE_DELTA["FT4"]` −0.30→−0.20, NUR datenbasiert. R1+Final-R1 **PUSH FREIGEBEN**. Tests →2348. NICHT gepusht. — **Vorgänger v0.98.61 Audio-Mithör-Monitor (🔊-Toggle, Diagnose)** (voller Workflow). Mike-Wunsch: per Ohr prüfen ob Betrieb ist — Gezwitscher hörbar + leere Empfangsliste = App-Problem (nicht leeres Band). Neu `core/audio_monitor.py`: RX-Audio (24k int16 mono) optional auf Lautsprecher; **Decoder unangetastet** (Wrapper `mw_radio._on_rx_audio`, Decoder zuerst); vorallokierter numpy-Ringpuffer (GC-frei, nicht-blockierend im VITA-49-Empfangsthread), sounddevice-Callback, Ausgabe **fest 48k** (×2 sample-and-hold, kein Pitch-Shift — 24k auf macOS nicht überall nativ), Underrun→Stille (read-Index nie über write). `active`=GIL-atomares bool, Lock um Ringpuffer. 🔊-Toggle in `rx_panel` neben NEUE, persistent (`settings["audio_monitor"]`, Auto-Start wenn zuletzt an), Start-Fehler→Button zurück+Info-Zeile, closeEvent→stop. **Reiner RX, kein TX, ANT1/ANT2 unberührt.** DeepSeek R1 (2🔴 48k+Fehler-Rückroll, 3🟠/🟡 Ringpuffer/Underrun/Lifecycle — eingearbeitet) + Final-R1 **PUSH FREIGEBEN** (7 Prüfpunkte, 0 Bugs, keine Races). Tests 2324→**2339** (+15 `test_audio_monitor.py`, Fake-sounddevice). FEATURES §22. NICHT gepusht. — **Vorgänger v0.98.60 P171: DT-Korrektur auf EINEN globalen Wert (nur FT8 misst)** (voller Workflow). Mike: wenige Stationen auf FT4/FT2 verschlechtern die DT; die gelernte Korrektur (~0.26s) ist die konstante FlexRadio-RX-Latenz (VITA-49) → modus-/band-unabhängig (modus-/slot-abhängige Fensterlage liegt separat in `decoder._DT_OFFSETS`). Field-Beweis: fast alle FT4/FT2-Werte matchen FT8, nur FT4_20m=0.0451 war ein 1-Stationen-Artefakt (`_MIN={FT8:3,FT4:1,FT2:1}`). `core/ntp_time.py`: EIN globaler `_correction` (`{"dt_correction_s":0.26}`), gelernt NUR aus FT8 (`update_from_decoded` → `if _mode!="FT8": return False`), FT4/FT2 lesen/No-op; `set_mode`/`set_band` BEHALTEN den Wert (kein Per-Key-Laden); `MIN_STATIONS=3`, Clamp 1.0; Migration alt→global = Median der FT8-Werte in `_load_saved()` (in-memory, kein Import-Write → FT4_20m=0.045-Müll fällt raus); Seed = Hardware-Default 0.26. Entfernt: `_mode_key`/`_load_for_current_key` (Cross-Modus-Fallback P48-B)/per-Modus-`_MIN`/`_MAX_CORR`/`_log_load_dedup`. Decode-Unabhängigkeit geklärt (Korrektur ist KEINE Decode-Voraussetzung). DeepSeek R1 (3🔴 aufgelöst) + Final-R1 **PUSH FREIGEBEN**. Tooling: `deepseek_review.py max_tokens 16K→32K` (v4-pro-Reasoning sprengte das Limit → leere Antwort). Kein TX-Eingriff, ANT1/ANT2 unberührt. Tests 2332→**2324** (entfernte Sonderpfade). **⚠️ Mike: App NEU STARTEN** (Migration baut dt_corrections.json um). NICHT gepusht. — **Vorgänger v0.98.59 P170: Upload-Move mergt bei Namens-Kollision** (voller Workflow). Mike-Field: 205 hochgeladene QSOs blieben in der „neu"-Liste, weil das Verschieben nach `hochgeladen/` bei gleichnamiger Tagesdatei übersprang (11/12 neu-Dateien hatten einen Zwilling in hochgeladen/ — Phase-1-Migrationsfolge; strukturell wiederkehrend, da tägliche Logdateien gleich heißen). Fix `log/adif.py:merge_adif_files(src,dest)`: bei Kollision Records dedupliziert (`(CALL,QSO_DATE,TIME_ON)` = Export-Key) an die vorhandene hochgeladen/-Datei **anhängen**, dann neu-Datei löschen. **Datensicherheit:** dest byte-erhaltend (nur Anhang), `open(...,newline="")` (keine Newline-Übersetzung), striktes utf-8 + `<EOH>`-Check → bei kaputter Datei ValueError, Aufrufer lässt beide stehen; atomar (Temp + os.replace); idempotent. `_handle_qrz_file_results` mergt statt zu skippen. **Die 205 räumen sich von selbst:** App neu starten → 1× QRZ-Upload → Dups → je Datei gemergt, neu/ leert sich. DeepSeek R1 (Datensicherheit) + Final-R1 NICHT FREIGEBEN (Newline-Byte-Erhalt) → behoben → Final-R1b **PUSH FREIGEBEN**. Reine Dateioperation, kein TX-Eingriff. Tests 2324→**2332** (+8 `test_p170_upload_merge.py`, Kollisions-Skip-Test auf Merge umgestellt). NICHT gepusht. — **Vorgänger v0.98.58 P169 Phase 2: mode-genauer Worked-Filter (Call,Band,Mode) + Auto-Hunt-Transparenz** (voller Workflow). „Schon gearbeitet" unterscheidet jetzt die Betriebsart: eine auf 20m FT8 gearbeitete Station ist auf **20m FT4** und **15m FT8** wieder „neu" — NEUE-Filter (RX-Liste) UND Auto-Hunt band+mode-genau. Neuer Index `QSOLog._worked_band_mode: set[(call,band,mode)]` (additiv, alte Indizes bleiben), befüllt in `load_adif` (**effektiver Mode = SUBMODE wenn vorhanden, sonst MODE**, `.upper()`; FT4=MFSK+SUBMODE→„FT4", QRZ MODE=FT4→„FT4", FT8→„FT8"; **leerer Mode NIE indiziert**) + `add_qso(call,band,mode="")`; neue `is_worked_on_band_mode(call,band,mode)` (leerer/None mode-Param→False); `clear()` leert mit. `ui/mw_qso.py:657` Live-`add_qso` gibt `settings.mode` mit (Token-konsistent zum ADIF-Loader). `ui/rx_panel.py` NEUE-Filter band+mode-genau via **Provider-Callback** `set_band_mode_provider(lambda:(settings.band,settings.mode))` (lazy gelesen → eine Quelle, kein Staleness; bewusst gegen kombinierten Setter, vermeidet P102/P114-Sync-Bug-Klasse); kein Provider → call-only-Fallback. `core/auto_hunt.py` Worked-Filter `is_worked_on_band`→`is_worked_on_band_mode`; neues Signal `all_worked=Signal(str,str,int)` **entprellt** (`_all_worked_reported`, Reset NUR start/set_band/set_mode — NICHT pro Pick) → `main_window._on_auto_hunt_all_worked` → `qso_panel.add_info("Auto-Hunt: alle N Stationen auf {Band} {Mode} schon gearbeitet")`; Emit nur wenn vor Worked-Filter Kandidaten da waren. `ui/mw_radio.py:609` `auto_hunt.set_band` jetzt **IMMER** (auch bei inaktivem Auto-Hunt — sonst `_band` stale beim nächsten Start), `on_band_change()` nur wenn aktiv. **Land-Seltenheit bleibt mode-blind** (`_country_count`/`_country_band`/`_compute_priority` unberührt — keine P165-Regression). DeepSeek R1 (6 Findings, alle 🟡/⚪, 0 Blocker — F2 set_band-immer + F4 Debounce-nur-start/band/mode angenommen, F1 Provider→Setter + F3 _apply_filters-Trigger abgelehnt: Callback robuster, RX-Tabelle wird bei Wechsel ohnehin geleert) + Final-R1 **PUSH FREIGEBEN** (0 Bugs/Risiken, Token-Konsistenz+Debounce+keine Races bestätigt). Reine State-/Anzeige-Logik, **kein TX-Eingriff, ANT1/ANT2 unberührt.** Tests 2312→**2324** (+12 `test_p169_phase2.py`; 4 Test-Fakes/Mocks um `is_worked_on_band_mode` ergänzt). **Damit ist P169 komplett (Phase 1 + 2).** NICHT gepusht, Field-Test pending. — **Vorgänger v0.98.57 P169 Phase 1: adif/erfasst/ = einzige Worked-Quelle + ADIF-Import + Migration** (voller Workflow). Mike-Field: Auto-Hunt rief auf vollen Bändern „kein Ruf raus" (Debug-Log `all_worked_on_band` = P165-Filter greift, KEIN Bug); Analyse deckte ADIF-Unordnung auf (Worked-Index las nur 3/8 Ordner, nicht-rekursiv; frische QSOs zählten erst nach QRZ-Upload; 95 Stationen nur in nicht-geladenen Ordnern; doppeltes `adif/adif/`). Fix: EINE rekursiv gelesene Quelle **`adif/erfasst/{neu,hochgeladen,importiert}/`**. Migration `tools/migrate_adif_erfasst.py` (copy→SHA256-verify→delete + Backup-ZIP nach Appsicherungen/, idempotent, Nicht-ADIF bleibt): 75 .adi → 9647 (Call,Band) byte-genau erhalten, Altordner weg, Klassifikation Variante A (Historie→importiert/ = kein Re-Upload, frische→neu/, hochgeladene→hochgeladen/). 8 Code-Touchpoints: `load_directory`/`parse_all_adif_files`/`bulk_import_directory` mit `recursive=`; qso_log+LocatorDB+Logbuch lesen nur erfasst/ rekursiv; `AdifWriter`→`erfasst/neu/`; Upload-Kandidaten = nur `erfasst/neu/`, Move neu/→hochgeladen/; `export_all_records` aus erfasst/ (nur `SimpleFT8_LOG_*`); Diplome via `_all_records`; neuer `QSOLog.clear()` für Import-Reload-ohne-Doppelzählung. **Import-Button** (validieren ≥1 CALL → Kopie nach `erfasst/importiert/` → Index+Anzeige+LocatorDB reload via `adif_imported`); manuelles Kopieren nach erfasst/ wird beim Start ebenfalls erfasst. DeepSeek Migration-R1 (7 Findings → gehärtet) + Final-R1 **PUSH FREIGEBEN** (Upload-Filter wasserdicht, kein Re-Upload der 18k-Historie). Tests 2303→**2312** (+9 `test_p169_erfasst.py`). adif/ gitignored. **⚠️ Mike: App NEU STARTEN** (Migration baute adif/ um). (Phase 2 inzwischen erledigt — v0.98.58 oben.) — **Vorgänger v0.98.56 P168** (FT4 30s→15s Decode-Pfad-Fix, field-validiert QSO SV5AZK 15s-Takt): Mike-Field (2 QSOs, ms-Log): FT4-QSOs doppelt so langsam (unsere TX 30s statt 15s auseinander). FT4 = Zeitspar-Modus → 30s vergrault Stationen. Root Cause: Decoder weckte FT4 0,5s vor Slot-Ende (absolut 14,5s) → Decode ~0,24s NACH Boundary fertig → zu spät für Audio-Start des Folge-Slots (Boundary−0,8, FlexRadio-1,3s-TX-Buffer) → Encoder-Drift-Guard (`encoder.py:337`) sprang +2 Slots (15s) → 30s. Decoder weckt STRUKTURELL nach der Sende-Frist; Decode-Fenster hing an der Weckzeit (`audio_12k[-slot_samples:]`). **⚠️ 1. Versuch (nur `_WAKE_OFFSETS["FT4"]` 0,5→1,5) brach den EMPFANG (0 Decodes 15/20/30m, Field-Crash Mike) → zurückgerollt:** früheres Wecken verschob das gekoppelte Fenster → Signal aus dem ft8_lib-FT4-Sync-Fenster (+2,24 statt +1,24s). **Lehre: WAKE ≠ Fenster-Position ≠ DT.** Echter Fix `core/decoder.py`: 3 Größen ENTKOPPELT — `_WAKE_OFFSETS["FT4"]`=**1,5** (früh wecken) · neuer **`_WINDOW_OFFSETS`** {FT8:2.5,FT4:0.5,FT2:0.3} = Decode-Fenster **slot-ausgerichtet** [Slot−0,5;+7,0] via Helper **`_keep_window`** (end-verankert), Post-Signal-Rest **`_TAIL_PAD_SAMPLES`** (FT4=1,0s) **NACH `_preprocess_audio`** mit Nullen gefüllt (DeepSeek-Gold-Finding: sonst verfälschen Nullen RMS-Norm+Whitening) · **`_DT_OFFSETS` aus `_WINDOW_OFFSETS` abgeleitet** (NICHT _WAKE!) → FT4-DT konstant **1,0** unabhängig von WAKE. FT8/FT2 **bit-identisch** (tail=WAKE−WINDOW=0). DeepSeek Plan-R1 (Gold-Finding übernommen) + Final-R1 **PUSH FREIGEBEN** 0 Blocker; **Halluzination abgefangen** (DeepSeek wollte Parität /15 statt /7.5 — gegen `encoder.py:381` geprüft + verworfen, FT4 alterniert auf 7,5s). Hardware: reine Decoder-Timing-Logik, **kein TX-Eingriff, ANT1/ANT2 unberührt.** Tests 2290→**2303** (+13, `test_p168_ft4_timing.py` inkl. FT4-Positionierungs-Äquivalenz + FT8-Decode-Rundlauf). **Field-Test BESTANDEN (02.06. 10:25 UTC, QSO SV5AZK, TX exakt 15s-Takt, 6 Stationen Empfang). Push-Freigabe Mike ausstehend.** — Vorgänger v0.98.55 **P167 eingeschobenes QSO hing nach 1 Anruf** (Einschub synchron im qso_state-Handler → `_resume_cq_if_needed` überschrieb TX_CALL mit IDLE; Fix `QTimer.singleShot`-Defer), v0.98.54 **Logbuch-Tabelle: Datums-/km-Spalte chronologisch/numerisch sortieren** (`_SortableItem.__lt__` mit `_SORT_ROLE`; Claude-Catch DeepSeek-`super().__lt__` = PySide6 RecursionError → `self.text()<other.text()`; Tests +8), v0.98.53 **Diplome-Erweiterung: WAE + WPX + DXCC-Band-Tiefe + Ein-/Ausblenden** (Diplome-Feature war DXCC/WAC/WAS/WAZ; WAE=`CONT==EU`-Näherung Ziel 70, WPX=`wpx_prefix()` Ziel 300, DXCC-Challenge+5-Band-DXCC; 👁-Toggle pro Karte via `core/awards_prefs.py`; DLD/IOTA bewusst raus; Claude-Catch DeepSeek-WPX-`digit_parts[0]` falsch → kürzerer Teil; FEATURES §19; Tests +23), v0.98.52 **RX-Listen-Doppelklick = harter Auto-Hunt-Stop** (`_on_station_clicked(msg, hard_stop=True)` + Stop-Block oben; P164-QSO-Fenster bleibt sanft), v0.98.51 **Auto-Hunt DX-Scoring** (persönliche Land-Seltenheit aus 18k-Historie > Distanz > Signal; `_compute_priority`-Tupel, `SNR_FLOOR=-26`), v0.98.50 Bug 1 Anchor-Bleed, v0.98.49 Diplome. Ältere Versionen vollständig in **HISTORY.md** (grep nach Version).
...
Speicherung: ~/.simpleft8/dt_corrections.json → EIN globaler Wert
    {"dt_correction_s": 0.26}. v0.99.0: KEIN Auto-Lernen — gesetzt NUR per
    ⏱-Kalibrier-Knopf (ntp_time.calibrate, inkrementell aus FT8-Stationen,
    MIN_STATIONS=5, kein Negativ-Riegel). FT4/FT2 erben + _MODE_DELTA. set_mode/
    set_band behalten den Wert, leeren nur das Kalibrier-Fenster. _DT_OFFSETS oben
...
### DT-Korrektur (v0.99.0 — EIN globaler Wert, manueller Kalibrier-Knopf)
- **Pfad:** `~/.simpleft8/dt_corrections.json`
- **Format:** `{"dt_correction_s": 0.26}` — EIN globaler Wert für alle Modi/Bänder.
- **Gesetzt NUR per ⏱-Kalibrier-Knopf** (v0.99.0 — kein Auto-Lernen mehr).
  `calibrate()` misst inkrementell aus den FT8-Stationen (`record_samples` puffert
  `deque(maxlen=3)`, MIN_STATIONS=5, Median+MAD, voller Schritt, kein Negativ-
  Riegel). FT4/FT2 erben den Wert + `_MODE_DELTA`. Gleicht Hardware-Latenz UND
  driftende System-Uhr aus (Ferienhaus-iMac). `set_mode`/`set_band` behalten den
  Wert, leeren nur das Kalibrier-Fenster.
- **Migration** vom alten per-(Modus,Band)-Format automatisch in `_load_saved()`
  (globaler Wert = Median der FT8-Werte, in-memory). Seed-Kaltstart =
  Hardware-Default 0.26. Details: FEATURES §6 (v0.99.0-Block).
...
| `core/ntp_time.py` | `threading.Lock()` (`_lock`) | Korrekturwert + Kalibrier-Fenster (`_recent_samples`) |
...
- **dt_corrections.json** — EIN globaler Wert `{"dt_correction_s": 0.26}`. **v0.99.0: KEIN Auto-Lernen mehr** — der Wert wird NUR per ⏱-Kalibrier-Knopf gesetzt (`ntp_time.calibrate()`, inkrementell aus den FT8-Stationen, MIN_STATIONS=5, kein Negativ-Riegel). `record_samples()` puffert nur das gleitende FT8-Fenster (`deque(maxlen=3)`), `set_mode`/`set_band` leeren es. `_correction` ändert sich AUSSCHLIESSLICH in `calibrate`/`_load_saved`/`set_hardware_default`. Migration alt→global (Median der FT8-Werte) in `_load_saved()` bleibt. FEATURES §6 (v0.99.0-Block). **Falle:** `calibrate()`/`_save_current()` NIE außerhalb pytest aufrufen — schreibt Mikes echte `~/.simpleft8/dt_corrections.json` (conftest schützt nur unter pytest)
