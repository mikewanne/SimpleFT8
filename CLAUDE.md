...
**Aktueller Stand:** v0.98.50 (01.06.2026) — **Bug 1 (QSO-Log Anchor-Bleed) + Bug 3 (Meldungs-Kürzung)** behoben (voller Workflow). Im QSO-Verlauf blutete das Link-Format der klickbaren „← Empf.“-Einschub-Zeile (P164) auf nachfolgende Zeilen — eigene „→ Gesendet“-TX wurden optisch+klickbar zu Links. Fix in `ui/qso_panel.py`: `_append_colored`/`_append_two_color` setzen ein frisches `QTextCharFormat` via `setCurrentCharFormat()` statt nur `setTextColor()` (DeepSeek-Root-Cause ✓; sein Ein-Zeiler reichte NICHT, weil `setTextCursor(End)` das Anchor-Format neu lädt — kritisch geprüft + Test-First). 5 neue Tests (`tests/test_bug1_anchor_bleed.py`), Final-R1 ✅, 2233 grün. Bug 3: redundanter „Maus bewegen…“-Hinweis aus 3 `add_info`-Meldungen (`ui/main_window.py`) raus. **Bug 2 (Doppel-Uhrzeit RX/TX) diagnostiziert = Encoder-Slot-Übergangs-Race, harmlos, NICHT gefixt** (vorlegen → TODO/HANDOFF). **Lokal committet, NICHT gepusht.** — Vorgänger v0.98.49 **Diplome-Feature** (DXCC/WAC/WAS/WAZ im Logbuch-Tab, `core/awards.py`+`ui/awards_dialog.py`, on-demand QRZ-Export, Final-R1 PUSH FREIGEBEN). Ältere Versionen vollständig in **HISTORY.md** (grep nach Version).
