...
**Aktueller Stand:** v0.98.04 (25.05.2026) — P121 Multi-Radio-Refactor (Variante A): Hardware-Konstanten (`tx_buffer_s`, `rx_hardware_offset_default_s`, `tune_power_w`) aus Settings/hartcodiert in jede konkrete Radio-Klasse verschoben (FlexRadio, IC7300Interface, IC7100Interface). Class-Variable-Pattern als Duck-Typing (FlexRadio erbt QObject nicht ABC). IC-7300 + IC-7100 als Stub-Klassen — alle Methoden `NotImplementedError` mit P121-Verweis, Class-Variables gesetzt, instanziierbar ohne Hardware. `radio_factory` unterstützt jetzt alle drei Typen. Settings `radio_timing` als User-Override-Slot (Defaults vom Radio). Init-Reihenfolge umgedreht (`_init_radio_state` VOR `_init_core_components`). 5 TUNE-Pfade konsistent auf `radio.tune_power_w`. V4-pro 49-Cycle: 0 Halluzinationen, 1 Final-R1-Catch (3 weitere TUNE-Pfade). Tests: 1838.
