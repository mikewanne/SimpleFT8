...
**Aktueller Stand:** v0.87.1 (02.05.2026) — **Doku-Konsolidierung.** Komplette Doku-Reorganisation: `docs/explained/` ist jetzt Single Source of Truth fuer alle 20 User-Features (40 Files DE+EN). Alte UPPER_SNAKE_CASE-Files in `docs/` migriert (POWER_REGULATION → power-regulation, FREQUENCY_HISTOGRAM → cq-frequency, DX_TUNING → dx-tuning, bandpilot_help_* → bandpilot). 4 redundante geloescht (DIVERSITY, DT_CORRECTION) mit Inhalts-Merge. 5 neue User-Doku-Files (antenna-preference, waitlist, direction-map, locator-mining, auto-hunt). `ui/help_dialog.py:_FEATURES` von 11 auf 20 alphabetisch erweitert. ?-Button bekommt Tooltip. README mit allen Features + Doc-Tabelle vollstaendig (vorher 5 von 11 verlinkt). OMNI-TX-Aktivierungs-Methode aus Doku entfernt (PRIVAT). 11 atomare Commits. Final-R1 fand 1 valides Finding (Code-Verweis raus). Tests 593 → 616. **v0.87 (01.05.2026):** Bandpilot (RX-Modus-Empfehlung pro Band). Bei jedem Bandwechsel wählt der Bandpilot automatisch den RX-Modus (Normal / Diversity Standard / Diversity DX) der auf diesem Band die höchste Pooled-Mean-Stations-Anzahl liefert. Aggregation nach Mike's Kandidat A: `Normal_Mean` vs `(Diversity_Normal_Mean + Diversity_DX_Mean) / 2` — alle drei Stats-Pfade loggen die gleiche Metrik (mein anfänglicher Irrtum „DX zählt nur SNR<-10" wurde von Mike per Code-Check aufgeklärt). Schwellen: `MIN_DAYS=2`, `MIN_CYCLES=50` pro Modus. Cache pro Band 24h in `~/.simpleft8/bandpilot_summary.json`. Manueller Override greift einmal beim Rückwechsel zu diesem Band, dann Reset. Refactor in `_on_rx_mode_changed`: Diversity-Aktivierungs-Block in `_activate_diversity_with_scoring(scoring)` extrahiert — Dialog-Pfad und Bandpilot-Pfad teilen den Code. Settings-Dialog Tab „FT8 & Diversity" hat neue Checkbox + Pref-Combo (Auto/Standard/DX) + ?-Button mit sprachabh. QMessageBox-Hilfe (`docs/bandpilot_help_<de|en>.md`). **v0.86:** Fix G + Test-Coverage AC-1..AC-4 (+56, -5 Duplikate). **v0.85:** Dead-Code-Cleanup (AGC + input_sample_rate + sync_ntp + _ntp_offset, ~60 Zeilen). **v0.84:** Tertile-Analyse Statistik (Feature H). **v0.83:** Fix F Kalibrierungs-Dialog Auto-Close. **v0.82:** Doppel-Report-Bug-Fix (Fix E, drittes Decoder-Signal `cycle_finished`). **v0.81:** Fix D `on_decoder_finished` (Aufspaltung von `on_cycle_end`). **v0.80:** TX-DT-Drift-Fix (A1+A2+A3+B+C+Race) — Retry-Trigger 1 Slot früher, cancelable sleep, Counter-Reset zentral. **v0.79:** Bug-Cleanup. **v0.78:** OMNI-TX scharfgeschaltet. **v0.77:** App-Start Hardware-Dialog. **v0.76:** Settings-Tabs. **v0.75:** Auto-Hunt-Modus.
**Tests:** `./venv/bin/python3 -m pytest tests/ -q` → 616 passed (Qt-Smoke-Tests via `QT_QPA_PLATFORM=offscreen`)
