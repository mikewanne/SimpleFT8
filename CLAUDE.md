...
**Aktueller Stand:** v0.98.55 (02.06.2026) — **P167: eingeschobenes QSO (P164) hing nach 1 Anruf — Reentrancy-Fix** (voller Workflow). Mike-Field: P164-Einschub (fremde Station im QSO-Fenster geklickt) rief nach dem laufenden QSO nur EINMAL, dann Stillstand (kein Retry, Auto-Hunt-Pause blieb). Root Cause: `_p158_maybe_start_inserted_call` lief synchron im qso_state-Abschluss-Handler (`qso_timeout`/`qso_confirmed.emit` = DirectConnection); `start_qso` setzte TX_CALL, aber der Handler rief danach `_resume_cq_if_needed()` → `_set_state(IDLE)` und überschrieb TX_CALL → QSO hing. Fix `ui/mw_qso.py`: Einschub in nächsten Event-Tick defern (`_deferred_insert_msg` + `QTimer.singleShot(0, _execute_deferred_insert)`); State dann stabil IDLE → start_qso→TX_CALL bleibt. `_on_cancel` (HALT) nullt `_deferred_insert_msg` (Race-Schutz). DeepSeek Diagnose-R1 (Root Cause bestätigt, Option A) + Final-R1 **PUSH FREIGEBEN** 0 Blocker (1🟡 Caller-Queue-Race harmlos→TODO). Hardware: reine GUI-Ablauf-Steuerung, **kein TX, ANT1/ANT2 unberührt.** Tests 2286→**2290** (+4, `test_p167_insert_defer.py`). FEATURES §17. **Field-Test pending, NICHT gepusht.** — Vorgänger v0.98.54 **Logbuch-Tabelle: Datums-/km-Spalte chronologisch/numerisch sortieren** (`_SortableItem.__lt__` mit `_SORT_ROLE`; Claude-Catch DeepSeek-`super().__lt__` = PySide6 RecursionError → `self.text()<other.text()`; Tests +8), v0.98.53 **Diplome-Erweiterung: WAE + WPX + DXCC-Band-Tiefe + Ein-/Ausblenden** (Diplome-Feature war DXCC/WAC/WAS/WAZ; WAE=`CONT==EU`-Näherung Ziel 70, WPX=`wpx_prefix()` Ziel 300, DXCC-Challenge+5-Band-DXCC; 👁-Toggle pro Karte via `core/awards_prefs.py`; DLD/IOTA bewusst raus; Claude-Catch DeepSeek-WPX-`digit_parts[0]` falsch → kürzerer Teil; FEATURES §19; Tests +23), v0.98.52 **RX-Listen-Doppelklick = harter Auto-Hunt-Stop** (`_on_station_clicked(msg, hard_stop=True)` + Stop-Block oben; P164-QSO-Fenster bleibt sanft), v0.98.51 **Auto-Hunt DX-Scoring** (persönliche Land-Seltenheit aus 18k-Historie > Distanz > Signal; `_compute_priority`-Tupel, `SNR_FLOOR=-26`), v0.98.50 Bug 1 Anchor-Bleed, v0.98.49 Diplome. Ältere Versionen vollständig in **HISTORY.md** (grep nach Version).
