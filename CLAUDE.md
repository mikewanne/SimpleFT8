...
**Aktueller Stand:** v0.98.06 (25.05.2026) — P124 Hash-Call `<...>` kontextuell auflösen: Mike-Field-Bug + Mike's KISS-Idee — bei aktivem QSO mit RA9LL sendet Gegenstation `DA1MHH <...> R+10` (FT8 i3-Frame Hash-Marker für unaufgelösten 22-Bit-Hash), State-Machine `qso_state.py:604` verwirft den R-Report (caller mismatch) → Retry-Loop bis Timeout. Lösung: im aktiven QSO ist einziger sinnvoller Hash-Kandidat die Gegenstation → heuristisch ersetzen via `resolve_hash_in_msg(msg, qso.their_call)` in `_p124_resolve_hash_if_active_qso` (mw_cycle.py, VOR add_rx + on_message_received). Display zeigt plain Call, State-Machine matcht, Endlosschleife weg. **R1-F4 ORANGE-Catch:** ft8_lib/message.c:709 gibt 2 Marker-Formen (`<...>` UND `<CALL>` Brackets) — `is_hash_marker` deckt beide via Bracket-Test ab. 16 neue Tests inkl. T12 End-to-End-Bug-Fix-Beweis. Final-R1: PUSH FREIGEGEBEN. V4-pro 51-Cycle: 0 Halluzinationen. Macht P125 (Höflichkeits-73) wahrscheinlich überflüssig. Tests: 1867.
