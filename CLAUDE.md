...
**Aktueller Stand:** **v0.97.60 Session 19.05.2026 — P90: Connect-Worker hart abbrechen bei „ohne Radio weiter".** Mike-Field-Test 19.05.: trotz Klick auf „ohne Radio weiter" lief komplette FlexRadio-Connect-Sequenz durch (TCP, Phase 1+2, Panadapter, Slice, TX-Config) — App zeigte „Radio getrennt" aber Hardware war eingerichtet. P82-Slot-Guards griffen erst NACH `connected.emit()`. P90 Fix (DeepSeek-Variante A): neues `_abort_connect`-Flag analog `_abort_reconnect` in `radio/flexradio.py`. 5 Check-Punkte in `auto_connect()` + 4 in `connect()`. Bei Abort: `disconnect()` + `return False` + Log. `mw_radio._start_radio` Cleanup-Reihenfolge: `abort_connect` → `abort_reconnect` → `disconnect` → `thread.join(timeout=2.0)`. P82-Slot-Guards bleiben als 2. Schicht für Race-Fenster. Final-R1 „PUSH FREIGEBEN" 0 KP. **V4-pro 36-Cycle-Bilanz: 0 Halluzinationen.** Tests 1580→1588 (+8). Field-Test F1-F5 pending (Radio AN nötig). **P89 + P88 + P86 Field-Tests ✅** durch Mike bestätigt 19.05. nach App-Start.
