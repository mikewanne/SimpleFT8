...
**Aktueller Stand:** v0.98.51 (02.06.2026) — **Auto-Hunt DX-Scoring** (voller Workflow, 2 DeepSeek-Runden + Web-Recherche). Auto-Hunt bevorzugt jetzt seltene/weite/neue DX-Perlen statt lauter Europa-Nachbarn: **persönliche Land-Seltenheit** (aus 18k-QSO-Historie) ist Leitmaß → dann Entfernung → dann Signal nur als Stichentscheid. `core/auto_hunt.py`: `_score`→`_compute_priority` (lexikografisches Tupel `(R, band_new, -dist, -snr, slot)`, kleiner=höher), `country_rarity_class` (0 ATNO..4 >100), `_MIN_SNR=-21`→`SNR_FLOOR=-26` (schwaches DX ist die Perle, FT8 dekodiert bis ~−24 dB), Slot=letzter Tiebreaker, Vorfilter „gearbeitete Station skippen". `log/qso_log.py`: Länder-Zähler `_country_count`/`_country_band` + API `get_country_count`/`is_country_worked_on_band`. `ui/main_window.py`: lädt `adif/_backup_qrz_export/` (~18.329 QSOs, 0,47 s) + `set_my_grid`. Verifizierte Rangfolge: Falkland>San Marino(nah!)>Japan>USA>DL. **Mike-Kurswechsel** (DeepSeek-R1 „SNR lassen" verworfen) + Claude-Catch (additiver Distanzbonus zu schwach → Tupel-Ordnung). DeepSeek Final-R1 **PUSH FREIGEBEN** 0 Blocker. Hardware: nur Auswahl, TX bleibt ANT1. Tests 2233→**2245** (+12, `test_p165_dx_scoring.py`). **Field-Test pending, lokal committet, NICHT gepusht.** Restrisiko Phase 2: Sonderpräfixe (FT5 Kerguelen→Frankreich gezählt). — Vorgänger v0.98.50 Bug 1 Anchor-Bleed + Bug 3, v0.98.49 Diplome-Feature. Ältere Versionen vollständig in **HISTORY.md** (grep nach Version).
