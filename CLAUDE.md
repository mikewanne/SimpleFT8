...
**Aktueller Stand:** **v0.97.4 Session 12.05.2026 — P38 PID-Recycling-Schutz im Starter-Script, Tests 1136.** Mike-Screenshot 12.05. zeigte Starter blockt legitimen Neustart weil macOS SimpleFT8-PID an Chrome recycled hat. Fix in `starter.command:36-50` — `ps -p $LOCK_PID -o command=` + `grep "SimpleFT8.*main\.py"` hinter `kill -0`. Bei Recycling Lock loeschen + sauber starten. NICHT identisch mit altem „2 unsichtbare Instanzen"-Bug (separater Cleanup-Issue, Folgeprojekt). Workflow V1→V2→R1(0 KRITISCH)→V3. Plan-File `prompts/p38_pid_recycling_starter_r1.md`. Push pending.
