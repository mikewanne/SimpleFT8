...
**Aktueller Stand:** v0.98.01 (24.05.2026) — P116 FIFO-Sliding-Window Stats-Cleanup löst P52-90-Tage-Cleanup ab: Mike-Anforderung 24.05. (lange Funk-Pause → ALLE Stats weg + saisonale Antennen-Variation Sommer/Winter). FIFO pro `(Modus, Band, Stunde)`-Bucket mit N=30 (DeepSeek-Konsens). Antenna_QSO bleibt 90-Tage-Datum. Bandpilot-Cache wird invalidiert bei Lösch-Aktion. 3 neue Funktionen in `core/stats_cleanup.py`. Vor Eingriff: GitHub-Push + lokales Backup (Mike-Wunsch „nichts kaputt machen"). Final-R1 V4-pro STRENG-validiert, 10 Punkte, 0 Mängel. Tests: 1794.
