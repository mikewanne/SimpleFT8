...
**Aktueller Stand:** v0.98.52 (02.06.2026) — **RX-Listen-Doppelklick = harter Auto-Hunt-Stop** (voller Workflow). Doppelklick in der Empfangsliste = bewusste Übernahme → ALLES unterbrechen (CQ/QSO/Auto-Hunt) + sofort rufen, kein Resume (vorher lief Auto-Hunt weiter). `ui/mw_qso.py:_on_station_clicked(msg, hard_stop=True)` + Stop-Block GANZ OBEN (`stop_auto_hunt("manual_halt")` + P164-Merker verwerfen) → deckt alle Pfade ab (TX-Buffer/SWR/Slot-Lock/Normal). P164-QSO-Fenster-Klick bleibt sanft (`hard_stop=False`, pausieren+Auto-Resume; beide Pfade `mw_cycle._on_hunt_insert_clicked` + `mw_qso._p158_maybe_start_inserted_call`). Claude-Catch: DeepSeek-R1-🟠 `cancel()` verworfen (start_qso resettet schon, qso_state:297-330, P1.14). Final-R1 **PUSH FREIGEBEN** 0 Blocker/Findings. Hardware: TX bleibt ANT1. Tests 2245→**2255** (+10, `test_p166_rx_doubleclick_hardstop.py`). FEATURES §17. **Field-Test pending, lokal committet, NICHT gepusht.** — Vorgänger v0.98.51 **Auto-Hunt DX-Scoring** (persönliche Land-Seltenheit aus 18k-Historie > Distanz > Signal; `_compute_priority`-Tupel, `country_rarity_class`, `SNR_FLOOR=-26`; lädt `adif/_backup_qrz_export/`; Rangfolge Falkland>San Marino>Japan>USA>DL; Restrisiko FT5-Sonderpräfixe), v0.98.50 Bug 1 Anchor-Bleed + Bug 3, v0.98.49 Diplome. Ältere Versionen vollständig in **HISTORY.md** (grep nach Version).
