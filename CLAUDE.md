...
**Aktueller Stand:** v0.88 (04.05.2026) — **Bandpilot Stunden-Refactor.** Drei direkte Werte pro UTC-Stunde (Normal / Diversity Standard / Diversity DX) **ohne Aggregation** — ersetzt v0.87 ``(S+D)/2`` (R1: Std/DX nicht IID → Aggregat erzeugt Bias). Settings ``bandpilot_mode`` ∈ ``{off, auto, manual}``. Auto: Toleranz ``max(5%, 1 Sta)`` gegen aktuellen Modus. Manual: Dialog NUR wenn Top-1 != aktuell. Schwellen pro Stunde: ``MIN_DAYS_HOUR=3``, ``MIN_CYCLES_HOUR=20``. Cache ``~/.simpleft8/bandpilot_hourly.json`` (alter ``bandpilot_summary.json`` wird bei Migration geloescht). Settings-Migration idempotent: ``bandpilot_enabled=true`` → ``mode="auto"``, sonst ``"off"``. ``Settings.save()`` jetzt atomar. TX-Schutz mit Band-Konsistenz-Check (R1-Final): Wechsel verzoegert bis ``tx_finished``, bei Bandwechsel waehrend TX wird pending verworfen. ``BandpilotAutoToast`` (3s self-close, parent-zentriert, Frameless+Tool, ``WA_DeleteOnClose``) + ``BandpilotManualDialog`` (3 Buttons, 1/2/3-Marker, Top-1 gruen, ●-Marker fuer aktuellen). MD-Generator ``core/bandpilot_md.py`` schreibt ``auswertung/Bandpilot-<band>-FT8.md`` (24-Zeilen-Tabelle, atomic write). 13 atomare Commits. **v0.87.1 (02.05.2026):** Doku-Konsolidierung (40 Files DE+EN in `docs/explained/`). **v0.87:** Bandpilot v1 (Aggregat-Logik — durch v0.88 ersetzt). **v0.86:** Fix G + Test-Coverage. **v0.85:** Dead-Code-Cleanup. **v0.84:** Tertile-Analyse Statistik. **v0.83:** Fix F. **v0.82:** Fix E. **v0.81:** Fix D. **v0.80:** TX-DT-Drift-Fix. **v0.79:** Bug-Cleanup. **v0.78:** OMNI-TX scharfgeschaltet. **v0.77:** Hardware-Dialog. **v0.76:** Settings-Tabs. **v0.75:** Auto-Hunt-Modus.
**Tests:** `./venv/bin/python3 -m pytest tests/ -q` → 659 passed (Qt-Smoke-Tests via `QT_QPA_PLATFORM=offscreen`)
