...
**Aktueller Stand:** **v0.97.52 Session 18.05.2026 nach Compact — P80 Unified Gain Store (1 Messung pro Band für alle Modi: FT8/FT4/FT2 + Normal/Diversity Std/Diversity DX), Tests 1517.** Mike-Vorschlag: Hardware-Gain ist Antennen+Vorverstärker-Eigenschaft, modus-unabhängig. Neuer Store `~/.simpleft8/kalibrierung/presets.json` mit Key=Band only. `ant2_calibrated`-Feld trennt echte Diversity-Messung von Normal-Migration. 3 Quellen migriert (presets_standard.json + presets_dx.json + settings.normal_presets) via MAX(gain_timestamp). R1-V4-pro 7 Findings (2 ROT gefangen — F1 ant2_calibrated-Check verhindert Hardware-Fehlanwendung, F2 robuster Fallback). V4-pro 27-Cycle-Bilanz: 0 Halluzinationen. Push pending bis Field-Test F1-F6. **Vorgänger v0.97.51 P79 UI-Bundle** (Symbol-Auto-Detect + Modal-Kalibrierung raus + 3-Optionen-SWR-Text).
