...
**Aktueller Stand:** v0.89 (04.05.2026) — **Kalibrier-Pipeline Block 1.** Pipeline 6:50 → ~4:31 Min (-2:19 Min, -34 %). 5 atomare Commits ueber V1→V2→R1→V3-Workflow: ``ebddd3e`` Skip-First-Cycle entfernen (-15 s, ``ui/dx_tune_dialog.py:60`` + Logik in ``feed_cycle``), ``5662d76`` TUNE 5s→3s (6 Stellen in ``ui/mw_radio.py``), ``bea87f9`` Gain-Stufen 3→2 (``GAIN_VALUES = [10, 20]``, 18→12 Zyklen, -90 s, KISS ohne Fallback), ``3a4de56`` Phase 3 ``MEASURE_CYCLES`` 8→6 (R1-KRITISCH: ``mw_radio.py:798`` ueberschreibt sonst die Konstante zur Laufzeit), ``aec3706`` Preset-Cache 2h→6h. Tests 659 gruen (zwei Tests umbenannt + range-Anpassung). Block 2 (Adaptiv-Stops, ROUNDS=2) DRINGEND als naechstes — Trigger „Block 2 starten", typisch ~3:20 Min Ziel. **v0.88 (04.05.2026):** Bandpilot Stunden-Refactor — drei direkte Werte pro UTC-Stunde ohne Aggregation, Settings ``bandpilot_mode`` ∈ ``{off, auto, manual}``, ``MIN_DAYS_HOUR=3``, ``MIN_CYCLES_HOUR=20``, TX-Schutz mit Band-Konsistenz-Check, atomarer Settings.save() + MD-Write, 13 atomare Commits. **v0.87.1:** Doku-Konsolidierung. **v0.87:** Bandpilot v1 (Aggregat-Logik — durch v0.88 ersetzt). **v0.86:** Fix G + Test-Coverage. **v0.85:** Dead-Code-Cleanup. **v0.84:** Tertile-Analyse Statistik. **v0.83:** Fix F. **v0.82:** Fix E. **v0.81:** Fix D. **v0.80:** TX-DT-Drift-Fix. **v0.79:** Bug-Cleanup. **v0.78:** OMNI-TX scharfgeschaltet. **v0.77:** Hardware-Dialog. **v0.76:** Settings-Tabs. **v0.75:** Auto-Hunt-Modus.
