...
**Aktueller Stand:** **v0.96.2 P5.OMNI-PATTERN-FIX-3 — Encoder-Pending-Queue + Slot-Boundary-Display, Code fertig + Final-R1 freigegeben, Field-Test pending (10.05.2026)** — Field-Test v0.96.1 deckte Pos-1-Encoder-Race auf (FT8 12.64s Audio + 1.3s FlexRadio-Buffer-Drain → `_is_transmitting=False` fällt :42.8-:44.5, Pos 1 :45 hat <1s Race-Window → 100% busy). Lösung Variante A (R1-bestätigt KISS): `core/encoder.py` Pending-TX-Queue + Verfall (`1.5*cycle_dur`). `transmit()` queut bei busy unter `_replace_lock` statt `return False`. `_tx_worker` Single-Pass via `_run_one_tx_pass` + Pending-Loop mit **F1-KRITISCH abort-Check** (`if _abort_event.is_set(): return` VOR Re-Trigger — Cold-Start-Test entdeckt). Issue A: `main_window.py:760` Slot-Boundary statt Wall-Time löst Pos-4-RX-E + `:44 [O] Horche` mit. **6 atomare Commits:** C1 (`229e98c` encoder), C2 (`96f5714` tests), C3 (`333411a` main_window), C4 (`955aeb0` tests), C5 (`31f2f41` APP_VERSION), C5a (`5408534` Final-R1+Werte-Fix), C6 (Doku). **Final-R1: „Push freigegeben"** (0 KRITISCH + 1 SOLLTE-FIX gefixt). **Test-Bilanz: 1020 → 1034 grün** (+14 — V3 prognostizierte ~1029, +5 Bonus). **Trigger nach Compact:** „omni v0.96.2 field-test" → KI lädt project_p5_omni_pattern_fix3.md + V3 §6 7-Punkte-Plan F1-F7. Push pending — v0.95.16-0.96.2 + P2-Tool gesammelt. **Plan-Files:** prompts/p5_omni_pattern_fix3_v[1,2,3].md + _r1.md + _final_r1.md.
**Tests:** `./venv/bin/python3 -m pytest tests/ -q` → **1034 passed** v0.96.2 (Qt-Smoke-Tests via `QT_QPA_PLATFORM=offscreen`)
