...
**Aktueller Stand:** v0.98.05 (25.05.2026) — P122 Auto-Hunt-Stop-Defer bei aktivem QSO: Mike-Field-Bug — `timer_expired`/`mouse_inactive_5min`/`totmann_expired` brachen mitten in laufendem Stations-Ruf ab → „rufe laufen ins leere". Lösung: 3 Stop-Reasons werden bei aktivem QSO (`qso_sm.state not in IDLE/TIMEOUT/CQ_CALLING/CQ_WAIT`) deferiert bis QSO-Ende (✓/✗/HALT). `manual_halt`/`swr_block`/`band_change`/etc. greifen weiterhin sofort. `core/auto_hunt.py` + 1 Callback-Param + `flush_pending_stop()`. 3 Aufrufe in `mw_qso.py` (Aktion VOR P81-Meldung). 13 neue Tests inkl. FIFO/Idempotenz/Backward-Compat. R1: 5 Findings (3 ROT — alle V2-Fehler von mir, korrigiert), Final-R1: PUSH FREIGEGEBEN 0 Mängel. V4-pro 50-Cycle: 0 Halluzinationen. Tests: 1851.
