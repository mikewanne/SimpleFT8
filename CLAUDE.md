...
**Aktueller Stand:** **v0.97.50 Session 18.05.2026 — P76-C TUNE-bad setzt Band-Marker proaktiv (P63-Luecke nach Mike-Field-Test mit Limit=1.5), Tests 1484.** Mike testete P76-A-Fix, entdeckte Folge-Bug: TUNE-bad logged korrekt „SWR 2.0 > Limit 1.5", aber CQ-Button blieb klickbar → TX startet → P53-Watchdog feuert ERST beim TX-Versuch. Mike-Spec: schon vor TX-Klick sperren. Root: `_tune_post_swr_check` else-Branch hatte Kommentar „AC7 P63: Marker bleibt rot" — irrefuehrend, im Code KEIN `_swr_blocked_bands.add(band)`. P63 nahm an Marker existiere schon vom Watchdog, was bei TUNE OHNE vorherigen TX-Alarm nicht zutrifft. **Fix (Variante A, KISS, ~10 LOC):** Marker-Set im else-Branch wenn `tuner_present=True`, Pattern aus `_on_swr_alarm`. Marker-Set VOR `is_auto`-Abzweigung → konsistent fuer manuellen + Auto-Tune-Pfad. Text-Variante: mit Tuner „⚠ Band X gesperrt — Manueller TUNE zum Freischalten nach Antennen-Check", ohne Tuner „⚠ Tuner konnte nicht matchen — Antenne pruefen". Was bleibt klickbar (Mike-Spec): Bandwechsel (`_on_band_changed` blockt nicht), manueller TUNE (`_on_tune_clicked` hat keinen Marker-Check → Freischalt-Pfad). Pre-Checks (CQ Z.298, OMNI, Hunt, Station-Click, Diversity-Preset) greifen ab jetzt auch bei TUNE-bad-Marker. **R1-V4-pro 12 Findings: 0 ROT/ORANGE, 4 🟡 Doku-Hinweise** (F07 Kommentar korrigieren ✓, F08 P63 AC7-Doku anpassen ✓, F10 AutoTuneDialog indirekt KISS-akzeptiert). Final-R1 V4-pro „PUSH FREIGEGEBEN" knapp & klar. **V4-pro 25-Cycle-Bilanz: 0 Halluzinationen.** APP_VERSION 0.97.49→0.97.50. Tests 1474→1484 (+10 P76-C T1-T10 inkl. T2 Auto-Tune-Pfad-Konsistenz, T4 P63-AC6-Regression, T10 Idempotenz). Plus 1 P75-T9-Anpassung (suchte alten „AC7 P63: Marker bleibt rot"-Kommentar → P76-C-Marker). Backup `Appsicherungen/2026-05-18_v0.97.49_vor_p76c/`. **Push pending** bis Field-Test F1-F5 (alle Radio-pflichtig: F1 CQ-Sperre, F2 OMNI, F3 Hunt, F4 Freischalt, F5 Bandwechsel).

**Vorgänger:** **v0.97.49 Session 18.05.2026 — P76-A SWR-Safety-Bug (False-OK durch FlexRadio-Clamp nach tune_off), Tests 1474.** Mike-Field-Test 17m vormittags: SWR 2.7 (>Limit 2.5) wurde als „TUNE OK SWR 1.0" 3× geloggt — SAFETY-relevant (User glaubte Band sicher fuer 70-W-QSO → Hardware-Schaden-Risiko). Voller V1→V2→R1→V3→Code→Final-R1-Workflow mit DeepSeek-V4-pro nach Compact. **Root:** `_tune_post_swr_check` Z.262 las `radio.last_swr` 2 s NACH `tune_off()`. FlexRadio Meter-Loop liefert ohne TX-Traeger Mess-Artefakte <1.0 die `_handle_meter` auf 1.0 clampt → `_last_swr` ueberschrieben → `1.0 <= 2.5 = limit` → false-OK + P63-Marker NICHT gesetzt. **Fix (Variante A, KISS, ~15 LOC):** State-Var `_tune_last_valid_swr: float | None = None`, Freeze `= radio.last_swr` direkt VOR `tune_off()` (TX noch aktiv → Wert valid), Read+Reset im Post-Check, FAIL bei None oder <1.0 (`swr_now = swr_limit + 1.0` zwingt else-Branch). KEIN Fallback auf `radio.last_swr` (R1-F5: wuerde Clamp-Bug reproduzieren). Reset auch im Disconnect-Branch (R1-F6 ROT Stale-State-Schutz). `<1.0` ebenfalls FAIL (R1-F13 physikalisch unmoeglich). **R1-V4-pro fand 3 Findings**, alle uebernommen — F6 war echter ROT-Bug den V2-Self-Review uebersah. Final-R1 V4-pro „PUSH FREIGEGEBEN" 0 KP, 1 🟡 nicht-blockierend. **V4-pro 24-Cycle-Bilanz: 0 Halluzinationen.** APP_VERSION 0.97.48→0.97.49. Tests 1463→1474 (+11 P76-A T1-T11 inkl. T10 functional Mike-Fall-Simulation, T11 None→FAIL-Pfad). Plus Helper-Anpassung in `test_p54_fix.py` + `test_p54_auto_tune.py` (je +1 Zeile `obj._tune_last_valid_swr = swr`). Backup `Appsicherungen/2026-05-18_v0.97.48_vor_p76a/` (3 Files). **Push pending** bis Field-Test F1-F5 (F1-F4 mit Radio).

**Vorgänger:** **v0.97.48 Session 18.05.2026 — P75 TUNE-Button-Bug + Style-Harmonisierung + Fenster-Konsolidierung (Variante A), Tests 1463.** Autonomer Workflow mit DeepSeek-V4-pro während Mike unterwegs. Mike-Field-Test nach P71 brachte 2 Beobachtungen + 1 Folge-Auftrag (P74-A): (1) TUNE-Knopf bleibt nach Auto-Stop visuell aktiv (Bug), (2) Style-Harmonisierung mit OMNI/CQ-Pattern (aktiv grün, inaktiv dezent), (3) Mike: „viele fenster die aufploppen verwirren" → Bandwechsel ohne Preset öffnet 2 Fenster nacheinander. **Voller V1→V2→R1→V3-Workflow.** R1-V4-pro fand 6 Findings, entschied bei Architektur-Wahl Variante A (Header-Banner) statt B (State-Machine-Refactor) — KISS-Prioritaet: 30 Min vs 3-4 h. **Findings:** F1 🟡 Button-Reset mit `blockSignals(True/False)` + `setChecked(False)` in `_tune_stop`, Token-Guard reicht als Race-Schutz. F2 ⚪ eigenes `_tune_btn_style`-Cluster (dezent-gelb inaktiv `rgba(60,50,0,0.55)` Text `#BBA060`, grün aktiv `rgba(0,150,0,0.75)` analog OMNI) — TUNE als Setup-Aktion farblich von TX-Aktionen abgegrenzt. F3 EMPF Variante A: DXTuneDialog `__init__` neuer Param `prev_tune_swr: float | None = None`, bei Nicht-None grüner Header-Banner „✓ TUNE OK — SWR X.X · jetzt 2 Min Gain-Messung läuft" als visueller Übergang Phase 1 → Phase 2. F4 🟡 SWR-bad-manueller-TUNE-Pfad: QMessageBox.warning ersetzt durch `qso_panel.add_info` (rote Live-Log-Zeile, kein Popup). F5 ⚪ FWDPWR-Anzeige im Status-Label beibehalten (Hobby-Funker findet's nützlich, R1-Bestätigung). F6 🟡 +3 Tests (Race-Token-Guard, Banner-None-Pfad, QMessageBox-Absence). 6 atomare Commits geplant. **V4-pro 23-Cycle-Bilanz: 0 Halluzinationen, klare KISS-Architektur-Entscheidung.** APP_VERSION 0.97.47→0.97.48. Tests 1453→1463 (+10). Backup `Appsicherungen/2026-05-18_v0.97.47_vor_p75/` (gezielt 5 Files). **Push pending** bis Mike Field-Test F1-F4 (F1 ohne Radio, F2+F3+F4 mit Radio).

**Vorgänger:** **v0.97.47 Session 18.05.2026 — P71 Auto-Tune Bundle (5 Bugs aus Mike-Field-Test 18.05.), Tests 1452.** Autonomer Workflow während Mike unterwegs. Mike-Field-Test 18.05. morgens brachte 5 Bugs/UX-Punkte aus AutoTuneDialog: (Bug 1) "TUNE Timeout 19s/15s" trotz SWR 1.9 bei 20m→17m — Backup-Timer `(duration_s+5)*1000`=20s eng gegen worst-case 15+6.5+2=23.5s. Fix: Konstante `_BACKUP_GRACE_S=12` (Phase B 6.5 + Post-Check 2 + Safety 3.5) → `(duration_s+12)*1000`. (Bug 2) Auto-Tune feuert beim App-Start ungewollt — Trigger-Pfad bei R1 nicht eindeutig identifiziert (H3 widerlegt: unconnected Signal in PySide6 wird verworfen). Belt-and-suspenders: Guard-Flag `_initial_band_set` in MainWindow + RFPreset-Anker-Check via neue `RFPresetStore.has_anchor(radio, band, watt)`. (Bug 3) Settings tune_duration_s 15/30 → 5/10/15 mit findData-Fallback + Settings.load()-Migration. (Bug 4) Title `band.lower()+mode` statt `band.upper()`, Status mit Live-FWDPWR via `_fwdpwr_samples[-1]` (KISS-Coupling F4). (Bug 5) 5 DONE-Logs (`[P54a] DONE OK/FAIL ...`) für swr_ok/swr_bad/disconnect/cancelled/timeout im 1-Zeilen-key=value-Format für grep-Diagnose. R1-V4-pro fand 5 Findings: F1 🔴 (Backup-Race), F2 🟠 (Guard-Flag), F3 🟡 (Migration), F4 🟡 (KISS-akzeptiert), F5 🟡 (Logging übernommen). Final-R1 pending. **V4-pro 22-Cycle-Bilanz: 0 Halluzinationen, 1 ROT-Bug F1 gefangen, F2 sauber H3 widerlegt.** APP_VERSION 0.97.46→0.97.47. Tests 1435→1452 (+17). Backup `Appsicherungen/2026-05-18_v0.97.46_vor_p71/`. **Push pending** bis Field-Test F1-F7 (F1+F3+F4 ohne Radio, F2+F5+F6+F7 mit Radio).

**Vorgänger:** **v0.97.46 Session 17.05.2026 — P69 Block-Bootstrap-Konfidenz-Intervalle für Diversity-Statistiken, Tests 1435.** Autonomer Workflow während Mike unterwegs. Aus TODO P69 (DeepSeek-Vorschlag aus GitHub-Review). `scripts/bootstrap_ci.py` NEU mit `compute_bootstrap_ci_diff_pct` (Block-Bootstrap über (date, hour)-Blöcke, 5000 Iterationen, Percentile-CI). Pooled Mean über alle Cycles aller gezogenen Blöcke matcht `_combo_summary_fair`-Logik. Unabhängiges Resampling pro Modus (kein paired-bootstrap). Threshold n < 15 → „insufficient", 15 ≤ n < 25 → „limited", n ≥ 25 → „ok". R1-V4-pro fand 6 Findings: F-DIV0 🔴 ROT (Division-by-zero abgefangen, max 10 Retries), F-RATIO1 🟠 (BCa verworfen, Variante B mit Percentile-CI + Threshold-Schutz reicht), F-THRESHOLD 🟠 (Schwelle 15/25 statt 5/10), F-ITER 🟡 (5000 statt 1000), F-TEST-DATA 🟡 (Mock-Loader für T12), F-CAVEAT-LANG 🟡 (präziser Wortlaut). **Wichtigste Erkenntnis: Daten haben sich seit alter README weiterentwickelt** — 40m Std fiel von +126% auf +62% (CI +32-+102%, signifikant), 40m DX von +123% auf +36% (CI +11-+70%, signifikant). 20m+30m CIs enthalten 0 → kein signifikanter Effekt nachweisbar. 6 Tabellen in README (DE+EN) auf neue Werte + CI gebracht. Plus Helper `scripts/print_ci_for_readme.py` für künftige Updates. PDF-Tabelle Seite 3 zeigt jetzt 2-zeilig „Punktschätzer + CI". V4-pro 21-Cycle-Bilanz: 0 Halluzinationen, 1 ROT-Bug gefangen. APP_VERSION 0.97.45→0.97.46. Tests 1415→1435 (+20). PDFs neu generiert. Push pending mit pending v0.97.40-45.

**Vorgänger:** **v0.97.45 Session 17.05.2026 — P54-FIX: echte 10W-Closed-Loop-Convergenz beim TUNE, Tests 1415.** Mike-Analyse: P54 (v0.97.44) hatte fälschlich „Slider 10 = 10W" hart für jedes Band gespeichert — Kalibrierungs-Philosophie der `RFPresetStore`-Tabelle kompromittiert. P54-FIX implementiert das eigentliche Konzept: zweiphasiger TUNE mit Closed-Loop in Phase B. **Phase A (`duration_s - 5`s):** Slider=10 fest, Tuner-Match. **Phase B (max 5s):** SWR-Check → wenn OK, `_tune_converge_to_target` iteriert proportional bis FWDPWR≈10W (max 5 Iter, Toleranz 1W). Speichert echten Slider-Wert in `rf_preset_store.save(band, 10, <konvergiert>)`. Plausibilität `rf ∈ [3..50]` — Werte ausserhalb verworfen. `_kruecken_skalierung` als Initial-Startwert: `anchor_rf × (target_w / anchor_watt) × 0.9` (Sicherheits-Faktor gegen nichtlineare PA, Mike-Spec). Krücke nur wenn genau 1 Stützpunkt — Closed-Loop ersetzt sie ab 1. konvergiertem QSO. Voller V1→V2→R1→V3→Code→Final-R1+Round 2 autonom. **R1-V4-pro fand 4 ROT-Bugs:** F1 `set_power` nach `_apply_rf_preset` (State-Sync), F2 Cancel-Race in Convergenz, F3 State-Var Init fehlte, F4 hart-Save war eigentlicher P54-Bug. **Final-R1 V4-pro fand 1 weiteren ROT-Bug:** Re-Entry-Race in `_tune_stop` durch Qt-Sub-Event-Loop (Cancel-Click parallel zum Stop-Pfad → doppelte `tune_off`). Fix: `_tune_stop_active`-Sperre, bei Re-Entry nur `_tune_convergence_cancelled=True`. **Final-R1 Round 2: „PUSH FREIGEGEBEN".** **V4-pro 20-Cycle-Bilanz: 0 Halluzinationen, 3 echte ROT-Bugs gefangen.** APP_VERSION 0.97.44→0.97.45. Tests 1395→1415 (+20 netto: 19 neue P54-FIX-Tests + 14 alte P54-Tests an neue rf-Plausibilitäts-Logik angepasst). Backup `Appsicherungen/2026-05-16_v0.97.44_vor_p54fix/`. **Push pending** bis Mike Field-Test F1-F8 (alle radio-pflichtig).
