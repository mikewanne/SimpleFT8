...
**Aktueller Stand:** v0.90 (04.05.2026) — **Mess-Pattern-Bug-Fix (KRITISCH).** ``core/diversity.py:86`` Mess-Phase auf fair 3:3 (``("A1","A1","A2","A2","A1","A2")``, Even+Odd-Paar je Antenne) korrigiert. Pre-v0.90 Pattern ``("A2","A1","A1","A2","A1","A1")`` war 4×A1 + 2×A2 (struktureller Bias zu ANT1, identisch mit OPERATE 70:30). Erklaert teilweise Mike's Beobachtung 4 % ANT2-Win-Rate auf 40 m FT8. 3 atomare Commits: ``473f164`` Pattern-Fix + 5 neue Tests (``test_measure_ratio_balanced``, ``_seamless_loop``, ``_closed_pairs_per_antenna``, End-to-End ``_evaluate_fair_yields_5050`` + ``_a1_dominant_yields_7030``), ``2a3c535`` Doku-Sync README+docs/explained DE+EN auf 6-Slot fair 3:3, ``<v0.90-doku>`` APP_VERSION + 4-Datei-Update. Voller Workflow V1→V2→R1→V3 (R1 fand 2 KRITISCH: End-to-End-Test, Bandwechsel-Race-Hinweis behalten). Pipeline ~4:31 Min unveraendert. Tests 664 gruen (= 659 + 5). Statistik-Disclaimer: alle Pre-v0.90 Diversity-Daten haben strukturellen Mess-Bias 4:2; Pooled-Mean +88 %/+124 % bleibt valide; absolute ANT2-Win-Rate war konservativ unterschaetzt. **🟡 Offen: mw_radio.py Bandwechsel-Race** — separater Workflow nach v0.90. **v0.89 (04.05.2026):** Kalibrier-Pipeline Block 1 — 6:50 → ~4:31 Min (-34 %), 5 atomare Commits ``ebddd3e`` Skip-First-Cycle, ``5662d76`` TUNE 5s→3s, ``bea87f9`` Gain-Stufen 3→2, ``3a4de56`` MEASURE_CYCLES 8→6, ``aec3706`` Preset-Cache 6h. **v0.88 (04.05.2026):** Bandpilot Stunden-Refactor — drei direkte Werte pro UTC-Stunde ohne Aggregation, Settings ``bandpilot_mode`` ∈ ``{off, auto, manual}``, ``MIN_DAYS_HOUR=3``, ``MIN_CYCLES_HOUR=20``, TX-Schutz mit Band-Konsistenz-Check, atomarer Settings.save() + MD-Write, 13 atomare Commits. **v0.87.1:** Doku-Konsolidierung. **v0.87:** Bandpilot v1 (Aggregat-Logik — durch v0.88 ersetzt). **v0.86:** Fix G + Test-Coverage. **v0.85:** Dead-Code-Cleanup. **v0.84:** Tertile-Analyse Statistik. **v0.83:** Fix F. **v0.82:** Fix E. **v0.81:** Fix D. **v0.80:** TX-DT-Drift-Fix. **v0.79:** Bug-Cleanup. **v0.78:** OMNI-TX scharfgeschaltet. **v0.77:** Hardware-Dialog. **v0.76:** Settings-Tabs. **v0.75:** Auto-Hunt-Modus.
**Tests:** `./venv/bin/python3 -m pytest tests/ -q` → 664 passed (Qt-Smoke-Tests via `QT_QPA_PLATFORM=offscreen`)
