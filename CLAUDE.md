...
**Aktueller Stand:** v0.98.00 (24.05.2026) — P115 Empfangsfenster bleibt bei RX-Mode-Switch/Kalibrierung/Sub-Toggle: Mike-Bug 24.05. „Empfangsfenster wird gelöscht obwohl gleiches Band → muss sich wieder füllen, das ist optisch unschön — wie ein Fortschrittsbalken beim Kopieren". P110 `clear_panels`-Parameter komplett aus 3 Funktionen entfernt, 5 Lösch-Code-Blöcke raus. `_enable_diversity`/`_disable_diversity`/`_activate_diversity_with_scoring` löschen nie mehr. Echte Lösch-Pfade reduziert auf 3: `_on_band_changed`, `_on_mode_changed`, `_on_rx_panel_toggled`. R1-F4 Catch: `qso_panel.log_view.clear()` in `_disable_diversity` BLEIBT (Chronik, nicht Empfangsfenster). 9 neue Tests + P110-Tests gelöscht. Tests: 1787.
