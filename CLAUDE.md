...
**Aktueller Stand:** v0.97.99 (24.05.2026) — P114 MODUS+BAND Status-Suffix bei eingeklappter Kachel: analog P97 Antenne/Radio jetzt auch die MODUS+BAND-Kachel mit dezentem Header-Suffix „— FT8 · 20m" wenn eingeklappt. `lbl_mb_status` in `_ModeBandCard`, Visibility-Toggle in `set_collapsed`, Methode `_refresh_modeband_status_label` in `ControlPanel:1779`, 2 Aufrufe in `_set_mode`/`_set_band` NACH emit. 6 neue Tests T16-T21. V4-pro R1-F2 als Halluzination identifiziert (falsches Zeilen-Zitat). Tests: 1785.
