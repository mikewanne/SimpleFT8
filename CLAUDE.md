...
**Aktueller Stand:** v0.98.03 (24.05.2026) — P118 Band-Aktivitäts-Script Berliner Zeit (DST-aware): Mike-Folge-Wunsch nach P117-Sichtung. Pro File-Datum via `zoneinfo.ZoneInfo("Europe/Berlin")` umgerechnet — Mai → UTC+2, Dezember → UTC+1, Wechsel-Tage automatisch (zoneinfo kennt komplette IANA-Datenbank). Neuer Helper `_utc_file_to_local_hour(date_str, utc_hour)`. Plot-Labels „Stunde (Berlin)" / „Hour (Berlin)". 2 neue DST-Tests (T11 Winter, T12 Helper-Direkt mit Wechsel-Tagen). Tests: 1806.
