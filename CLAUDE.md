...
**Aktueller Stand:** v0.98.49 (31.05.2026) — **Diplome-Feature (DXCC/WAC/WAS/WAZ)** im Logbuch-Tab (voller DeepSeek-Workflow). Button „Diplome“ öffnet read-only Dialog mit den 4 klassischen Diplomen, je „gearbeitet“ + „bestätigt (LoTW)“; DXCC mit echten ARRL-Marken (100/150/200/250/300/Honor Roll), WAC/WAS/WAZ nur Fortschritt + Badge (keine erfundenen Medaillen, R1b Option A: Ehrlichkeit > Gamification). `core/awards.py` + `ui/awards_dialog.py` NEU, Datenquelle = QRZ-Export `adif/_backup_qrz_export/` on-demand. Tests 2228 grün. **Lokal NICHT committet/gepusht — Final-R1 + Field-Test pending** (Details + offene Punkte → HANDOFF.md). — Frühere Versionen (v0.98.48 P164 · v0.98.45 P161 · … bis v0.98.12) sind vollständig in **HISTORY.md** dokumentiert; bei „wie war vX.YY?“ → grep HISTORY.
