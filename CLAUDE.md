...
**Aktueller Stand:** **v0.97.61 Session 20.05.2026 — P91: Dialog-Lifecycle-Crash-Fix (P90 Folge-Bug).** Mike-Field-Test P90 F1: SIGSEGV-Crash beim Klick auf „ohne Radio weiter". Stack zeigte Worker-Thread im `~QDialogWrapper` während GUI-Thread Qt-Events für selben Dialog dispatched — Use-After-Free. Ursache: Worker hielt `dlg = self._connect_dialog` (strong ref), P90's `thread.join` synchronisierte Worker-Termination auf Cleanup-Punkt → letzte Ref im Worker → Destruktor lief dort → Race. Fix (DeepSeek-Variante C): `import weakref` + `dlg_ref = weakref.ref(self._connect_dialog)` im Worker — KEINE strong ref mehr. Plus Cleanup-Block: `dialog_keepalive` Snapshot → `_connect_dialog = None` VOR join → `dialog_keepalive.deleteLater()` nach join → Refcount-Drop beim Funktions-Ende im GUI-Thread = SAFE. Final-R1 „PUSH FREIGEBEN" 0 KP. **V4-pro 37-Cycle-Bilanz: 0 Halluzinationen.** Tests 1588→1596 (+8 P91). Field-Test P90 F1-F5 wiederholen (Radio AN). **Vorgänger v0.97.60 P90** Connect-Worker-Abort. **P89 + P88 + P86 Field-Tests ✅** durch Mike 19.05.
