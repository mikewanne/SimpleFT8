...
**Aktueller Stand:** v0.98.02 (24.05.2026) — P117 Band-Aktivitäts-Übersicht-Script (Standalone): Mike-Use-Case PNG-Quick-Reference für den Park („wann ist welches Band typisch aktiv"). Neues Script `scripts/band_activity_summary.py` + Shell-Wrapper `banduebersicht.sh` im Root. Liniendiagramm alle Bänder × 24 UTC-Stunden, Pooled-Mean über vorhandene RX-Modi (durch ANZAHL Modi, nicht stur durch 3). KEIN App-Eingriff, kein Hook in main.py oder generate_plots.py (Mike-Spec „standalone, leichter wartbar"). 10 neue Tests. Live-Smoke-Test mit Mike-Daten: 4 Bänder geplottet. Tests: 1804.
