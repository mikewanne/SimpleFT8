...
**Aktueller Stand:** **v0.97.54 Session 19.05.2026 — Bundle M autonom: P83 Gain-Status-Zeile mit Verfalls-Counter („noch X Stunden", farbcodiert grün/orange/rot, aktionsgetrieben KEIN Timer, auch im Normal-Modus) + P85 ANT2-Win-% statt Stationsanzahl (Median 4 Zyklen, löst 1-Zyklus-Versatz-Inkonsistenz „3:12+50:50"). R1-V4-pro Pre-Code 8 Findings (alle eingebaut), Final-R1 „PUSH FREIGEBEN ✅" 0 KP. **V4-pro 30-Cycle-Bilanz: 0 Halluzinationen.** Tests 1533→1548. Field-Test F1-F7 pending (alle mit Radio). **Vorgänger v0.97.53 P81** Auto-Hunt-Stop-Meldung Defer.

**ALT (v0.97.53 P81):** Tests 1533. Mike-Field-Test: „⏸ Auto-Hunt gestoppt"-Meldung erschien mitten im QSO-Verlauf (zwischen RR73-Send und finalem ✓). Fix: Defer-Flag `_auto_hunt_stop_msg_pending` + Helper `_qso_active_for_msg_defer()`/`_flush_auto_hunt_stop_msg()`. Flush in 3 QSO-Ende-Pfaden: `_on_qso_confirmed_visual`, `_on_qso_timeout`, `_on_cancel` (HALT — R1-F1 ROT-Fix). Geister-Schutz beim Manual-Restart. R1-V4-pro 3 Findings (1 ROT F1 gefangen, 1 ORANGE dokumentiert, 1 GELB Naming+Test). Final-R1 „PUSH FREIGEGEBEN". **V4-pro 28-Cycle-Bilanz: 0 Halluzinationen.** Push pending bis Field-Test F1-F5. **Vorgänger v0.97.52 P80 Unified Gain Store** (1 Messung pro Band für alle Modi).
