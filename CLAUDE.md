...
**Aktueller Stand:** v0.97.98 (23.05.2026) — P113 Stale-Gain-Warning bei Bandwechsel: dezenter Statusbar-Toast wenn Gain-Kalibrierung > 14 Tage alt (P74-B Phase 1 aus DeepSeek-Spec 18.05.). Ergänzt das bestehende 6h-„Re-Mess nötig"-Label in dx_info um eine prominentere Schicht mit konkretem Alter („17 Tage alt"). Neue Konstante `STALE_GAIN_WARNING_DAYS = 14` in `core/preset_store.py:45`, neue Methode `_check_stale_gain_warning` in `ui/mw_radio.py:1428`, 2 Aufrufe in `_on_band_changed` (Diversity vor return + Normal nach `_update_statusbar`). 9 neue Tests, autonomer Workflow. Tests: 1779.
