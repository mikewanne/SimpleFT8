...
Pipeline:
  VITA-49 Audio (24kHz)
    ...
    → Noise-Floor-basierte Normalisierung (median → target 300)
    → DT-Korrektur (Audio-Buffer-Shift)
    → DC-Remove + Spectral Whitening (Overlap-Add FFT, Median)
            ...
            for offset_samples in SLIDE_OFFSETS:
                ...
                for r in results:
                    ...
                    if key and key not in seen:
                        ...
                        # 2) Buffer-Offset: Decode-Loop wacht _WAKE_OFFSETS[mode]s vor
                        #    Slot-Ende auf → Buffer startet _WAKE_OFFSETS[mode]s VOR
